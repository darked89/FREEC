...
const complex operator/(const complex& lhs,
                        const complex& rhs);
const bool    operator==(const complex& lhs,
                         const complex& rhs);
const bool    operator!=(const complex& lhs,
                         const complex& rhs);
const complex operator+(const complex& lhs);
...
const complex operator+(const complex& lhs,
                        const double & rhs);
const complex operator+(const double & lhs,
                        const complex& rhs);
const complex operator-(const complex& lhs,
                        const complex& rhs);
const complex operator-(const complex& lhs,
...
********************************************************************/
double vdotproduct(const double *v0,
                   int           stride0,
                   const double *v1,
                   int           stride1,
                   int           n);
complex vdotproduct(const complex *v0,
                    int            stride0,
                    const char    *conj0,
                    const complex *v1,
                    int            stride1,
...
void vmove(double       *vdst,
           int           stride_dst,
           const double *vsrc,
           int           stride_src,
           int           n);
void vmove(complex       *vdst,
           int            stride_dst,
           const complex *vsrc,
           int            stride_src,
           const char    *conj_src,
...
void vmoveneg(double       *vdst,
              int           stride_dst,
              const double *vsrc,
              int           stride_src,
              int           n);
void vmoveneg(complex       *vdst,
              int            stride_dst,
              const complex *vsrc,
              int            stride_src,
              const char    *conj_src,
...
void vadd(double       *vdst,
          int           stride_dst,
          const double *vsrc,
          int           stride_src,
          int           n);
void vadd(complex       *vdst,
          int            stride_dst,
          const complex *vsrc,
          int            stride_src,
          const char    *conj_src,
...
void vsub(double       *vdst,
          int           stride_dst,
          const double *vsrc,
          int           stride_src,
          int           n);
void vsub(complex       *vdst,
          int            stride_dst,
          const complex *vsrc,
          int            stride_src,
          const char    *conj_src,
...
void vmul(double *vdst,
          int     stride_dst,
          int     n,
          double  alpha);
void vmul(complex *vdst,
          int      stride_dst,
          int      n,
          double   alpha);
void vmul(complex *vdst,
...
********************************************************************/
double  vdotproduct(const double *v1,
                    const double *v2,
                    int           N);
complex vdotproduct(const complex *v1,
                    const complex *v2,
                    int            N);
...
public:
  ...
  template_1d_array(const template_1d_array& rhs)
  {
    ...
    if (rhs.m_iVecSize !=
        0) setcontent(rhs.m_iLow, rhs.m_iHigh, rhs.getcontent());
  }
  ...
  void setbounds(int iLow, int iHigh)
  {
    ...
    if (Aligned) m_Vec = (T *)ap::amalloc((size_t)(m_iVecSize * sizeof(T)), 16);
    else m_Vec = new T[(size_t)m_iVecSize];
  }
  ...
  {
    if ((iStart > iEnd) || wrongIdx(iStart) ||
        wrongIdx(iEnd)) return raw_vector<T>(0, 0, 1);
    else return raw_vector<T>(m_Vec + iStart - m_iLow, iEnd - iStart + 1, 1);
  }

  const_raw_vector<T>getvector(int iStart, int iEnd) const
  {
    if ((iStart > iEnd) || wrongIdx(iStart) ||
        wrongIdx(iEnd)) return const_raw_vector<T>(0, 0, 1);
    else return const_raw_vector<T>(m_Vec + iStart - m_iLow, iEnd - iStart + 1,
                                    1);
  }
...
public:
  ...
  template_2d_array(const template_2d_array& rhs)
  {
    ...
    if (rhs.m_iVecSize != 0)
    {
      ...
      for (int i = m_iLow1; i <= m_iHigh1; i++)
        for (int j = m_iLow2; j <= m_iHigh2; j++) operator()(i, j) = rhs(i, j);
  ...
  void setcontent(int iLow1, int iHigh1, int iLow2, int iHigh2, const T *pContent)
  {
    ...
    for (int i = m_iLow1; i <= m_iHigh1; i++, pContent += m_iHigh2 - m_iLow2 + 1)
      for (int j = m_iLow2; j <= m_iHigh2;
           j++) operator()(i, j) = pContent[j - m_iLow2];
  ...
  raw_vector<T>getcolumn(int iColumn, int iRowStart, int iRowEnd)
  {
    if ((iRowStart > iRowEnd) || wrongColumn(iColumn) || wrongRow(iRowStart) ||
        wrongRow(iRowEnd)) return raw_vector<T>(0, 0, 1);
    else return raw_vector<T>(&((*this)(iRowStart, iColumn)),
                              iRowEnd - iRowStart + 1,
                              m_iLinearMember);
  }
  ...
  raw_vector<T>getrow(int iRow, int iColumnStart, int iColumnEnd)
  {
    if ((iColumnStart > iColumnEnd) || wrongRow(iRow) ||
        wrongColumn(iColumnStart) ||
        wrongColumn(iColumnEnd)) return raw_vector<T>(
        0,
        0,
        1);
    else return raw_vector<T>(&((*this)(iRow, iColumnStart)),
                              iColumnEnd - iColumnStart + 1,
                              1);
  }
  ...
  {
    if ((iRowStart > iRowEnd) || wrongColumn(iColumn) || wrongRow(iRowStart) ||
        wrongRow(iRowEnd)) return const_raw_vector<T>(0, 0, 1);
    else return const_raw_vector<T>(&((*this)(iRowStart, iColumn)),
                                    iRowEnd - iRowStart + 1,
                                    m_iLinearMember);
  }
  ...
  const_raw_vector<T>getrow(int iRow, int iColumnStart, int iColumnEnd) const
  {
    if ((iColumnStart > iColumnEnd) || wrongRow(iRow) ||
        wrongColumn(iColumnStart) ||
        wrongColumn(iColumnEnd)) return const_raw_vector<T>(0, 0, 1);
    else return const_raw_vector<T>(&((*this)(iRow, iColumnStart)),
                                    iColumnEnd - iColumnStart + 1,
                                    1);
  }
...
/*class dataset
   {
   public:
    dataset():nin(0), nout(0), nclasses(0), trnsize(0), valsize(0), tstsize(0),
       totalsize(0){};
...
int    minint(int m1,
              int m2);
double maxreal(double m1,
               double m2);
double minreal(double m1,
               double m2);
bool   fp_eq(double v1,
             double v2);
bool   fp_neq(double v1,
              double v2);
bool   fp_less(double v1,
               double v2);
bool   fp_less_eq(double v1,
                  double v2);
bool   fp_greater(double v1,
                  double v2);
bool   fp_greater_eq(double v1,
                     double v2);
} // namespace ap
