...
public:
  ...
  GenomeDensity();
  GenomeDensity(std::string const& mateFile,
                std::string const& inputFormat,
                std::string const& matesOrientation,
                std::string const& chrLenFile);
  ~GenomeDensity(void);
  void smooth();
  void printDensity(std::string key,
                    int         i,
                    int         j);
  void calculateLogRatio(GenomeDensity controlGD,
                         std::string   type);
...
#endif // ifndef __GENOME_DENSITY_H__
