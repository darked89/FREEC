...
void BAFpileup::makepileup(GenomeCopyNumber & sampleCopyNumber,
                           GenomeCopyNumber & controlCopyNumber,
                           std::string        sample_MateFile,
                           std::string        control_Matefile,
                           std::string        outputDir,
                           std::string        makeminipileup,
                           std::string const& mateFileName,
                           std::string const& inputFormat,
                           std::string const& matesOrientation,
                           std::string        pathToSamtools,
                           std::string        pathToSambamba,
                           std::string        SambambaThreads,
                           std::string        chrLenFileName,
                           std::string        controlName,
                           std::string        targetBed,
                           std::string        pathToBedtools,
                           std::string        fastaFile,
                           int                minQualPerPos)
{
  // create a .bed file with regions of interest to create a minipileup:
  // targeted + flanks for WES or all chromosomes for WGS:
  std::string bedFileWithRegionsOfInterest = outputDir + "_NewCaptureRegions" +
                                             ".bed";

  if (targetBed != "")
  {
    int flanks = calculateFlankLength(mateFileName,
                                      inputFormat,
                                      matesOrientation,
                                      pathToSamtools,
                                      pathToSambamba,
  ...
  {
    createBedFileWithChromosomeLengths(bedFileWithRegionsOfInterest,
                                       chrLenFileName,
                                       true);
  }
  pathToBedtools_ = pathToBedtools; // /*
  string intersected = intersectWithBedtools(makeminipileup,
                                             outputDir,
                                             bedFileWithRegionsOfInterest,
                                             chrLenFileName);
  string sampleOutFileName = createPileUpFile(outputDir,
                                              pathToSamtools,
                                              pathToSambamba,
                                              SambambaThreads,
                                              sample_MateFile,
  ...
  if (controlName.compare("") != 0) {
    string controlOutFileName = createPileUpFile(controlName,
                                                 pathToSamtools,
                                                 pathToSambamba,
                                                 SambambaThreads,
                                                 control_Matefile,
...
float BAFpileup::calculateFlankLength(std::string const& mateFileName,
                                      ...
                                      std::string        SambambaThreads)
{
  ...
  // getMateOrientation(matesOrientation_str); // will not consider read
  // orientation to increase speed
  char *line_buffer;
  FILE *stream;
  ...
  if ((mateFileName.substr(mateFileName.size() - 3,
                           3).compare("bam") == 0) ||
    ...
    if (mateFileName.substr(mateFileName.size() - 3, 3).compare("bam") == 0) {
      if (pathToSambamba != "")     {
        command = pathToSambamba + " view -t " + SambambaThreads + " " +
                  mateFileName;
      }    else    {
    ...
    while (((line_buffer =
               getLine(buffer, MAX_BUFFER, stream,
                       line)) != NULL) && j < numberOfReadsToCheck) {
...
void BAFpileup::calculateNewBoundaries(std::string targetBed,
                                       int         flanks,
                                       std::string bedFileWithRegionsOfInterest)
{
    ...
    while (std::getline(file, line)) {
      ...
      myfile << strs[0] << "\t" << atoi(strs[1].c_str()) - flanks <<
        "\t" << atoi(strs[2].c_str()) + flanks << "\t" << strs[0] << ":"
             << atoi(strs[1].c_str()) - flanks << "-" << atoi(strs[2].c_str()) +
        flanks << "\t" <<
...
void BAFpileup::createBedFileWithChromosomeLengths(
  std::string bedFileWithRegionsOfInterest,
  std::string chrLenFileName,
  bool        doesNeedChrPrefix) {
  ...
  for (unsigned int i = 0; i < chr_names.size(); i++)  {
    if (doesNeedChrPrefix) myfile << "chr" << chr_names[i] << "\t" << "1" <<
        "\t" << lengths[i] << "\n";
    else myfile << chr_names[i] << "\t" << "1" << "\t" << lengths[i] << "\n";
  }
  myfile.close();
}

std::string BAFpileup::intersectWithBedtools(std::string makeminipileup,
                                             std::string outputDir,
                                             std::string bedFileWithRegionsOfInterest,
                                             std::string chrLen)
{
  ...
  if ((makeminipileup.substr(makeminipileup.size() - 3,
                             3).compare("vcf") == 0) ||
      (makeminipileup.substr(makeminipileup.size() - 6,
                             6).compare("vcf.gz") == 0)) {
    intersected = outputDir + "_SNPinNewCaptureRegions.vcf";
  }

  string command = pathToBedtools_ + " intersect -a " + makeminipileup + " -b " +
                   bedFileWithRegionsOfInterest + " > " + intersected;
  ...
  while (std::getline(file, line))  {
    nb_snp++;
    std::vector<std::string> strs = split(line, '\t');
    myfile << strs[0] << "\t" << atoi(strs[1].c_str()) - 1 << "\t" <<  atoi(
      strs[1].c_str()) << "\t" << strs[3] << "\t" << strs[4] << "\n";
  }
...
std::string BAFpileup::createPileUpFile(std::string outputDir,
                                        ...
                                        int         minQualPerPos)
{
  ...
  if (pathToSambamba != "")    {
    string samtools_arg = "--samtools -f " + fastaFile + " -d 8000 -Q " +
                          int2string(minQualPerPos) + " -q 1 -l " + intersected;
    command = pathToSambamba + " mpileup -t " + SambambaThreads + " -o " +
              minipileup + " " + control_MateFile + " " + samtools_arg;
  }  else   {
    command = samtools_path + " mpileup -f " + fastaFile + " -d 8000 -Q " +
              int2string(minQualPerPos) + " -q 1 -l " + intersected + " " +
              control_MateFile + " > " + minipileup; // discard reads wit 0
                                                     // mapping quality
  }
...
/*
   std::vector < std::vector<float> >BAFpileup::computeBAF(GenomeCopyNumber &
      sampleorcontrol, std::string minipileup, std::string outputDir,
       std::string filename)
   {
    ...
    for ( it=sampleorcontrol.chrCopyNumber_.begin() ; it !=
       sampleorcontrol.chrCopyNumber_.end(); it++ )
        {
            numberofObjects++;
        }
       ...
       sampleorcontrol.chrCopyNumber_.end(); it++ )
        ...
        while (std::getline(file,line) && (k < numberofObjects))
            ...
            if (chrtmp == "chr" + it->getChromosome())
            {
                ...
                while (line[i] != '\t')
                    {
                    if ((char)line[i] == reference_basetmp || (char)line[i] ==
                       (reference_basetmp+32) || ((char)line[i] ==
                       (reference_basetmp-32)))
                        {
                        B_nb[k][l]++;
                        }
                    if ((char)line[i] == alternate_basetmp || (char)line[i] ==
                       (alternate_basetmp+32) || ((char)line[i] ==
                       (alternate_basetmp-32)))
                        {
                        A_nb[k][l]++;
                        }
                    i++;
    ...
    ofstream myfile;
    std::vector < std::vector<float> > BAF = vector < vector<float> >
       (numberofObjects);
    ...
    for ( it=sampleorcontrol.chrCopyNumber_.begin() ; it !=
       sampleorcontrol.chrCopyNumber_.end(); it++ )
        ...
        {
            BAF[j].push_back(((float)B_nb[j][i]) / ((float)A_nb[j][i] +
               (float)B_nb[j][i]));
        }
        j++;
    }
    ...
    for ( it=sampleorcontrol.chrCopyNumber_.begin() ; it !=
       sampleorcontrol.chrCopyNumber_.end(); it++ )
        ...
        for (int i = 0; i < chr[j].size(); i++)
        {
            myfile << chr[j][i] << "\t" << snp_pos_pileup[j][i] << "\t" <<
               AB_nb[j][i] << "\t" << B_nb[j][i] << "\t" << A_nb[j][i] << "\t"
               << BAF[j][i]<<"\n";
        }
        j++;
    }
