                      ...
                      ap::real_1d_array& work)
{
  ...
  for (i = i1; i <= i2 - 1; i++)
  {
    ...
    jps = j1 + ips - i1;
    l   = i2 - i;
    ap::vmove(&work(1), 1, &a(ips, j), a.getstride(),
              ap::vlen(1, l));
    ap::vmove(&a(ips, j), a.getstride(), &a(i, jps), 1,
              ap::vlen(ips, i2));
    ap::vmove(&a(i, jps), 1, &work(1), 1,
              ap::vlen(jps, j2));
  }
}
                          ...
                          double                   beta)
{
  ...
  if (!trans)
  {
    ...
    {
      v =
        ap::vdotproduct(&a(i, j1), 1, &x(ix1), 1, ap::vlen(j1, j2));
  ...
  if (!transa && transb)
  {
    if (arows * acols < brows * bcols)
    {
      for (r = bi1; r <= bi2; r++)
      {
        ...
        {
          v =
            ap::vdotproduct(&a(l, aj1), 1, &b(r, bj1), 1, ap::vlen(aj1, aj2));
          c(ci1 + l - ai1,
            cj1 + r - bi1) = c(ci1 + l - ai1, cj1 + r - bi1) + alpha * v;
        }
      }
      return;
    }
  ...
  if (transa && transb)
  {
    ...
    else
    {
      for (l = aj1; l <= aj2; l++)
      {
        ...
        for (r = bi1; r <= bi2; r++)
        {
          v = ap::vdotproduct(&work(1),
                              1,
                              &b(r, bj1),
                              1,
                              ap::vlen(1, k));
          c(ci1 + l - aj1,
            cj1 + r - bi1) = c(ci1 + l - aj1, cj1 + r - bi1) + alpha * v;
        }
      }
      return;
    }
