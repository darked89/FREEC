...
public:
  ...
  void readCopyNumber(std::string const& mateFileName,
  ...
  void   calculateBreakpoints(double breakPointThreshold,
                              int    breakPointType);
  void   calculateBAFBreakpoints(double breakPointThreshold,
                                 int    breakPointType);
  void   calculateSDAndMed(int ploidy,
                           std::map<float, float>& sds,
                           std::map<float, float>& meds);
  void   calculateSDs(int ploidy,
                      std::map<float, float>& sds);  // only SDs
  float  calculateVarianceForNormalCopy(int ploidy); // OLD
  void   calculatePloidy(int minCNAlength);
  double calculateXiSum(int ploidy);                 // calculates
                                                     // sum_{i}{(med_i-supposedValue_i)
                                                     // / SQRT(var_i)}   var_i =
                                                     // pi/2/n*sd_i*2 which
  ...
  void   calculateCopyNumberMedians(int  minCNAlength,
                                    bool noisyData,
                                    bool CompleteGenomicsData);
  double calculateMedianRatioAround(float interval,
                                    float around);
  double calculateMedianAround(float interval,
                               float around);
  void   calculateSomaticCNVs(std::vector<EntryCNV>controlCNVs,
                              int                  controlPloidy);
  int    calculateMedianReadCountPerWindow();
  int    calculateSDReadCountPerWindow(int mean);
  void   calculateReadCountQuartiles(int& lower,
                                     int& upper);

  double calculateMedianAround(GenomeCopyNumber& controlCopyNumber,
                               float             interval,
                               float             around);
  float  evaluateContamination();
  float  evaluateContaminationwithLR();

  void   printRatio(std::string const& outFile,
  ...
  void   printCopyNumber(std::string const& chr,
                         std::ofstream    & file);
  void   printCopyNumber(std::string const& chr,
                         std::string const& outFile);
  void   printCGprofile(std::string const& outFile);
  void   printCGprofile(std::string const& chr,
                        std::ofstream    & file);
  void   printCNVs(std::string const& outFile);
  void   printBAF(std::string const& outFile,
                  SNPinGenome      & snpingenome,
                  std::string const& matefile = "");
  void   printBAF(std::string const& chr,
                  std::ofstream    & file,
                  SNPatChr         & snpAtChrom,
                  std::string        myName,
                  std::string const& matefile);
  ...
  int                  getPloidy();
  ChrCopyNumber      & getChrCopyNumberAt(int index) {
  ...
  void        setSambamba(std::string const& pathToSambamba,
                          std::string const& SambambaThreads_);
  bool        ifHasBAF();
  void        setSex(std::string sex);
  void        setSeekSubclones(bool seekSubclones);

  int         findWinNumber(int                position,
                            std::string        myName,
                            std::string const& matefile);
...
private:
  ...
  void fillMyHash(std::string const& mateFileName,
                  std::string const& inputFormat,
                  std::string const& matesOrientation,
                  int                windowSize,
                  int                step,
...
struct GenomeCopyNumberCalculateBreakpointArgWrapper : public ThreadArg {
  ...
  GenomeCopyNumberCalculateBreakpointArgWrapper(
    GenomeCopyNumber& genomeCopyNumber,
    double            breakPointThreshold,
    int               breakPointType) : genomeCopyNumber(genomeCopyNumber),
    breakPointThreshold(
      breakPointThreshold), breakPointType(breakPointType) {}
};
...
struct GenomeCopyNumberReadMateFileArgWrapper : public ThreadArg {
  ...
  GenomeCopyNumberReadMateFileArgWrapper(SNPinGenome      & snpInGenome,
                                         std::string const& mateFile,
                                         const std::string& inputFormat,
                                         int                minimalTotalLetterCountPerPosition,
                                         int                minimalQualityPerPosition)
    : snpInGenome(snpInGenome), mateFile(mateFile),
    inputFormat(inputFormat), minimalTotalLetterCountPerPosition(
      minimalTotalLetterCountPerPosition), minimalQualityPerPosition(
      minimalQualityPerPosition), p_genomeCopyNumber(NULL), windowSize(0),
    step(0) {}
  ...
  GenomeCopyNumberReadMateFileArgWrapper(SNPinGenome      & snpInGenome,
    ...
    inputFormat(inputFormat), minimalTotalLetterCountPerPosition(
      minimalTotalLetterCountPerPosition), minimalQualityPerPosition(
      minimalQualityPerPosition), p_genomeCopyNumber(&genomeCopyNumber),
    chrLenFileName(chrLenFileName), windowSize(windowSize), step(step), targetBed(
      targetBed) {}
};
