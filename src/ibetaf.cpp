...
double incompletebeta(double a, double b, double x)
{
  ...
  ap::ap_error::make_assertion(ap::fp_greater(a, 0) && ap::fp_greater(b, 0),
                               "Domain error in IncompleteBeta");
  ap::ap_error::make_assertion(ap::fp_greater_eq(x, 0) && ap::fp_less_eq(x, 1),
                               "Domain error in IncompleteBeta");
  ...
  if (ap::fp_less(a + b,
                  maxgam) &&
      ap::fp_less(fabs(y), maxlog) && ap::fp_less(fabs(t), maxlog))
  {
    t = pow(xc, b);
    t = t * pow(x, a);
    t = t / a;
...
double invincompletebeta(double a, double b, double y)
{
  ...
  i = 0;
  ap::ap_error::make_assertion(ap::fp_greater_eq(y, 0) && ap::fp_less_eq(y, 1),
