...
public:
  ...
  bool isMainThread() const {
    return pthread_self() == main_pthread;
  }
...
private:
  ...
  ThreadPoolManager(unsigned int max_threads,
...
public:
  ...
  Thread(void * (*wrapper)(void *),
         ThreadArg   *arg,
         bool         delete_arg,
         unsigned int num) : wrapper(wrapper), arg(arg), delete_arg(delete_arg),
    num(num) {
    state = NOT_STARTED;
    tid   = 0;
...
# endif // ifdef _WIN32
  ...
  long long duration() const {
    ...
    long long tv_usec =
      (state ==
       FINISHED ? (unsigned long long)(tv_finished.tv_sec - tv_started.tv_sec) *
       1000000 + (tv_finished.tv_usec - tv_started.tv_usec) : -1);
    pthread_mutex_unlock(&mp);
    return tv_usec;
  }
...
public:
  ...
  void addThread(void * (*wrapper)(void *), ThreadArg *arg,
                 bool delete_arg = true) {
    thread_list.push_back(new Thread(wrapper, arg, delete_arg,
                                     thread_list.size() + 1));
  }
...
private:
  ...
  ThreadPool(ThreadPoolManager *thrPoolManager,
             const std::string& name) : thrPoolManager(thrPoolManager),
    name(name) {}
};
