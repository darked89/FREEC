...
ChrCopyNumber::ChrCopyNumber(int                windowSize,
                             int                chrLength,
                             std::string const& chrName) {
...
ChrCopyNumber::ChrCopyNumber(int                windowSize,
  ...
  if (targetBed == "")     {
    ...
    if (file.is_open())     {
      ...
      // length_ should end up equal to the number of exons matching this
      // chromosome.
      length_ = 0;
      size_t tried = 0;
      ...
      while (std::getline(file, line))   {
        // Avoid catching a carriage return if the file uses windows-style line
        // endings
        if ((line.size() != 0) && (line[line.size() - 1] == '\r')) line.erase(
            line.size() - 1);

        size_t offset = 0;

        // Treat 'chr1' and '1' equivalently
        if (line.substr(0, 3) == "chr") offset += 3;
        ...
        if (line.substr(chrstart, offset - chrstart) == chromosome_) {
          ...
          if (offset !=
              geneoff) genes_names.push_back(line.substr(geneoff,
                                                         offset - geneoff));
          else {
      ...
      if (int(tried) != length_) {
        std::cerr << "Warning: skipped " << (tried - length_) <<
          " lines due to formatting problems\n";
      }
      ...
      cout << "Number of exons analysed in chromosome " << chromosome_ << " : " <<
        exons_Countchr_ << "\n";
      cout << "Average exon length in chromosome " << chromosome_ << " : " <<
        meanTargetRegionLength << "\n";

      if (meanTargetRegionLength < 30) {
        cerr <<
          "WARNING: check your file with targeted regions: the average length of targeted regions is unexpectedly short\n";
      }
    } else {
  ...
  //            for (int i = 0; i<exons_Countchr; i++)
  //            {
  //            coordinates_[i] = atoi(coordinatesTmp_[i].c_str()); //Chaque
  // début d'exons
  //            }
  //        cout << "Number of exons analysed in chromosome "<< chromosome_ << "
  // : " << exons_Countchr << "\n";
...
void ChrCopyNumber::mappedPlusOneAtI(int i, int step, int l) {
  if (l == -1)  {
    ...
    if ((int)readCount_.size() <= pos) {
      ...
      cout << "Reaching end of file for chr " << chromosome_ << ", position " <<
        i << "\n";
...
{
  if (i < int(genes_names.size())) return genes_names[i];
...
int ChrCopyNumber::getEndAtBin(int i) {
  if ((int)ends_.size() > i) return ends_[i];
  else return coordinates_[i] + windowSize_ - 1;
}
...
void ChrCopyNumber::setNotNprofileAt(int i, float value) {
  if (i < int(notNprofile_.size())) notNprofile_[i] = value;
...
void ChrCopyNumber::removeLowReadCountWindows(ChrCopyNumber control,
                                              const int     RCThresh) {
  if (length_ != control.getLength()) {
    cerr <<
      "Warning: control length is not equal to the sample length for chromosome "
         <<
      chromosome_ << "\n";
    cerr << "Sample: " << length_ << " windows; control: " <<
      control.getLength() << " windows\n";
    return;
  }
...
void ChrCopyNumber::fillInRatio(bool islog) {
  if ((int)ratio_.size() != length_) ratio_.resize(length_);

  for (int i = 0; i < length_; i++) {
    if ((readCount_[i] >= 0) &&
        !((mappabilityProfile_.size() > 0) &&
          (mappabilityProfile_[i] <= minMappabilityPerWindow))) {
      if (islog) {
...
void ChrCopyNumber::calculateRatioLog(ChrCopyNumber control,
                                      const double *a,
                                      const int     degree) {
  if ((int)ratio_.size() != length_) ratio_.resize(length_);
  ...
  for (int i = 0; i < length_; i++) {
    if ((control.getLength() > i) && (control.getValueAt(i) != 0)) {
      if ((mappabilityProfile_.size() == 0) ||
          (mappabilityProfile_[i] > minMappabilityPerWindow)) {
        ratio_[i] = readCount_[i] /
                    polynomial(control.getValueAt(i), a, 1, degree);

        if (ratio_[i] < 0) ratio_[i] = NA;
      } else ratio_[i] = NA;
    } else
    if (readCount_[i] == 0)
      // ratio_[i] = 1;
    ...
    // cout << readCount_[i] << "\t" << control.getValueAt(i) << "\t" <<
    // ratio_[i] << "\n";
  }
}

void ChrCopyNumber::calculateRatio(ChrCopyNumber control,
                                   const double *a,
                                   const int     degree) {
  if ((int)ratio_.size() != length_) ratio_.resize(length_);
...
void ChrCopyNumber::calculateRatio(ChrCopyNumber control, double a0, double a1) {
  ...
  for (int i = 0; i < length_; i++) {
    if ((control.getLength() > i) && (control.getValueAt(i) != 0) &&
        (readCount_[i] > 0) && (control.getValueAt(i) > 0))
      if ((mappabilityProfile_.size() == 0) ||
          (mappabilityProfile_[i] >
           minMappabilityPerWindow)) ratio_[i] =
          float(readCount_[i] / (control.getValueAt(i) * a0 + a1));
      else ratio_[i] = NA;
...
void ChrCopyNumber::calculateRatio(ChrCopyNumber control,
  ...
  for (int i = 0; i < length_; i++) {
    if ((control.getLength() > i) && (control.getValueAt(i) != 0))
      if ((mappabilityProfile_.size() == 0) ||
          (mappabilityProfile_[i] >
           minMappabilityPerWindow)) ratio_[i] = readCount_[i] /
                                                 control.getValueAt(i) /
                                                 normalizationConst;
      else ratio_[i] = NA;
    else
    if (readCount_[i] == 0)
...
void ChrCopyNumber::setAllNormal() {
  ...
  for (int i = 0; i < length_; i++) {
    if ((readCount_[i] != NA) && (i < int(notNprofile_.size())) &&
        (notNprofile_[i] > 0)) {
      ratio_[i] = 1;
    } else {
      ratio_[i] = NA;
...
void ChrCopyNumber::recalculateRatio(double *a, int degree) {
  ...
  for (int i = 0; i < length_; i++) {
    if (ratio_[i] != NA) {
      ...
      if (mappabilityProfile_.size() > 0) {
        if ((x > 0) && (mappabilityProfile_[i] > minMappabilityPerWindow)) // if
                                                                           // ((x>0)&&(notNprofile_[i]!=0))
          ratio_[i] = ratio_[i] / polynomial(x, a, 1, degree);
        else ratio_[i] = NA;
      } else {
        if ((x > 0) && (notNprofile_[i] > minMappabilityPerWindow)) // if
                                                                    // ((x>0)&&(notNprofile_[i]!=0))
          ratio_[i] = ratio_[i] / polynomial(x, a, 1, degree);
        else ratio_[i] = NA;
      }
    ...
    if ((ratio_[i] != NA) && (ratio_[i] < 0)) ratio_[i] = 0;  // this happens if
                                                              //  polynomial(x,a,b,c,d)
                                                              // < 0
  }
}
    ...
    if (readCount_[i] != NA) {
      ...
      if (uniqueMatch) {
        ...
        if ((x > 0) && (mappabilityProfile_[i] > minMappabilityPerWindow)) // if
                                                                           // ((x>0)&&(notNprofile_[i]!=0))
          ratio_[i] = readCount_[i] /
                      polynomial(x, a, 1, degree) / mappabilityProfile_[i];
        else ratio_[i] = NA;
      } else {
        // use a threshold, but correct using notN profile
        if (mappabilityProfile_.size() > 0) {
          if ((x > 0) && (mappabilityProfile_[i] > minMappabilityPerWindow)) // if
                                                                             // ((x>0)&&(notNprofile_[i]!=0))
            ratio_[i] = readCount_[i] /
                        polynomial(x, a, 1, degree) / notNprofile_[i];
          else ratio_[i] = NA;
        } else {
          if ((x > 0) && (notNprofile_[i] > minMappabilityPerWindow)) // if
                                                                      // ((x>0)&&(notNprofile_[i]!=0))
...
//				ratio_[i] =
// readCount_[i]/polynomial(x,a,b,c,d)/notNprofile_[i];
//			else
//				ratio_[i] = NA;
//		} else {
//			ratio_[i] = NA;
...
//			ratio_[i] = 0; //this happens if  polynomial(x,a,b,c,d)
// < 0
//	}
// }
...
void ChrCopyNumber::recalculateRatio(float constant) {
  for (int i = 0; i < length_; i++)
    if (ratio_[i] != NA) ratio_[i] /= constant;
}
...
void ChrCopyNumber::recalculateRatioWithContam(float contamination,
                                               float normGenytype,
                                               bool  isLogged) { // normGenytype==1
                                                                 // if AB,
                                                                 // normGenytype==0.5
                                                                 // if A
  if (!isLogged) {
    for (int i = 0; i < length_; i++)
      if (ratio_[i] != NA) {
        ...
        // //correct only for ploidy 2
        ratio_[i] =
          (ratio_[i] * (1 - contamination + 2 * contamination / ploidy_) -
           contamination * normGenytype / ploidy_ * 2) / (1 - contamination);

        if (ratio_[i] < 0) ratio_[i] = 0;
      }
  } else {
    for (int i = 0; i < length_; i++)
      if (ratio_[i] != NA) {
        ...
        ratio_[i] =
          (realCopy * (1 - contamination + 2 * contamination / ploidy_) -
           contamination * normGenytype / ploidy_ * 2) / (1 - contamination);
...
int ChrCopyNumber::calculateBreakpoints(double threshold,
                                        int    normalChrLength,
                                        int    breakPointType) {
  int chrLen = calculateBreakpoints_general(threshold,
                                            length_,
                                            ratio_,
                                            bpfinal_,
                                            normalChrLength,
...
int ChrCopyNumber::calculateBAFBreakpoints(double threshold,
                                           int    normalChrLength,
                                           int    breakPointType) {
  ...
  // add detected breakpoints to the breakpoints detected using copy number
  // profiles
...
int ChrCopyNumber::getCoveredPart(int breakPointStart, int breakPointEnd) { // for
                                                                            // exome-seq:
                                                                            // get
                                                                            // length
                                                                            // of
...
void ChrCopyNumber::calculateCopyNumberMedian(int  ploidy,
                                              int  minCNAlength,
                                              bool noisyData,
                                              bool CompleteGenomicsData,
                                              bool isLogged) { // create median
                                                               // profiles using
                                                               // 'bpfinal_' and
                                                               // store them in
                                                               // medianProfile_,
  ...
  if (ploidy != ploidy_) {
    cerr <<
      "..Warning: in calculateCopyNumberMedian() class's ploidy is different from "
         << ploidy << "\n";
    ploidy_ = ploidy;
  }
  ...
  for (int i  = 0; i < (int)bpfinal_.size(); i++) {
    ...
    int notNA                    = 0;
    string BAFValuesInTheSegment = ""; // here we merge all SNP Values of this
                                       // segment
        ...
        if (isBAFpresent && (BAF_[j] != NA) && (BAFvalues_[j] != "")) {
          ...
          if (BAFValuesInTheSegment != "") {
            if (BAFvalues_[j].compare(BAFvalues_[j - 1]) != 0) {
              string suffix =
                findSmallestSuffix(BAFvalues_[j - 1], BAFvalues_[j]);

              if (suffix != "") BAFValuesInTheSegment += ";" + suffix;

              // BAFValuesInTheSegment += ";"+BAFvalues_[j];
            }
          }
          else BAFValuesInTheSegment = BAFvalues_[j];
        }
      }
    }
    ...
    if (isBAFpresent && (notNA > 100) && (locMedian < 1) && noisyData) {
      ...
      if (numberofBAFpoints < threshold) ifHomoz = true;
      cout << "chr" << chromosome_ << ":" << breakPointStart << "-" <<
        breakPointEnd << " totalCount=" << totalCount << " notNA=" << notNA <<
        " numberofBAFpoints=" << numberofBAFpoints << " ratio=" << 1. *
        numberofBAFpoints / threshold << " hom:" << ifHomoz << "\n";
    }

    // check is totalCount is notNA is greater than minCNAlength
    if ((totalCount < minCNAlength) && (i < (int)bpfinal_.size() - 1)) { // merge
                                                                         // this
                                                                         // fragment
                                                                         // with
                                                                         // next
                                                                         // one
      // do nothing, do to the next cycle, keep the same position of
      // "breakPointStart"
    } else {
      // fragment_lengths_.push_back(totalCount);
      // if (notNA <= (totalCount+0.5)/2) { // no more than 1/2 of NA windows
      if ((notNA == 0) || (notNA < sqrt(float(totalCount)))) { // instead of
                                                               // (notNA == 1 &&
                                                               // totalCount>2)
                                                               // meaning that
                                                               // only one point
        ...
        if (isLogged) median = pow(2, median);
        ...
        if (isBAFpresent) {
          ...
          //                        medianBAF = NA;
          // medianBAF = NA;
          getBAFinfo(BAFValuesInTheSegment,
                     median * ploidy,
                     estimatedBAF,
                     fittedBAF,
                     medianBAFSym,
      ...
      // }
      // else if
      // (round_by_ploidy(median,ploidy)==round_by_ploidy(previousMedian,ploidy))
      // {

      //	//join two segments
      //	medianValues_.pop_back();
      //	median = (median+previousMedian)/2;			//approximatelly
      // true...
      //	notNA += fragmentNotNA_lengths_.back();
      //	fragmentNotNA_lengths_.pop_back();
      //	float sdLocal = (sd_.back()+sd(data,median))/2;
      //  //approximatelly true...
      //	sd_.pop_back();
      ...
      if (isBAFpresent) {
        ...
        // cout << "..Control: adding "<<medianBAFSym<< " to a fragment with
        // median*ploidy="<<median*ploidy<< "\n";
      }
  ...
  for (int i = 1; i < int(bpfinal_.size()); i++) fragment_lengths_.push_back(
      bpfinal_[i] - bpfinal_[i - 1]);
...
// void ChrCopyNumber::calculateCopyNumberMedian(int ploidy){ //create median
// profiles using 'bpfinal_' and store them in medianProfile_, info about
// medians themselves is stored in medianValues_ and about SD in sd_, lengths of
// fragments in bpLengths_
//	int breakPointStart = 0;
//	int breakPointEnd;
//	float median;
...
//		int totalCount = breakPointEnd-breakPointStart+1;
//		//fragment_lengths_.push_back(totalCount);
//		if (notNA <= (totalCount+0.5)/3) { // no more than 2/3 of NA
// windows
//			median = NA;
//		} else {
//			median = get_median(data); //including the last
// point!!!! Ask Kevin or check if it is correct!!
//		}
//
//		//check the previous median: may be one can merge the two
// regions?
//
//		if(totalCount==1 && median == NA && breakPointStart != 0){
//			median = previousMedian;
...
// old version
void ChrCopyNumber::calculateCopyNumberMedian() { // create median profiles
                                                  // using 'bpfinal_' and store
                                                  // them in medianProfile_,
                                                  // info about medians
                                                  // themselves is stored in
                                                  // medianValues_ and about SD
                                                  // in sd_, lengths of
                                                  // fragments in bpLengths_
  int   breakPointStart = 0;
  int   breakPointEnd;
  ...
  for (int i  = 0; i < (int)bpfinal_.size(); i++) {
    ...
    if (notNA <= (totalCount + 0.5) / 2) {
      ...
      median = get_median(data); // including the last point!!!! Ask Kevin or
    ...
    for (int j = breakPointStart; j <= breakPointEnd;
         j++) medianProfile_[j] = median;
...
int ChrCopyNumber::nextNoNAIndex(int i1, int ploidy, int min_fragment) {
  for (int i = i1 + 1; i < (int)medianValues_.size(); i++)
    ...
    if (getLevelAt(i, ploidy) >= 0) // !!! >= and not > 0.
      if (fragment_lengths_[i] > min_fragment) return i;
...
float ChrCopyNumber::nextNoNAMedian(int i1, int ploidy) {
  for (int i = i1 + 1; i < (int)medianValues_.size(); i++)
    if (getLevelAt(i, ploidy) > 0) return medianValues_[i];
...
int ChrCopyNumber::nextNoNALength(int i1, int ploidy) {
  for (int i = i1 + 1; i < (int)medianValues_.size(); i++)
    if (getLevelAt(i, ploidy) > 0) return fragment_lengths_[i];
...
int ChrCopyNumber::getNumberOfGoodFragments() {
  ...
  for (int i = 0; i < (int)fragmentNotNA_lengths_.size(); i++)
    if (fragmentNotNA_lengths_[i] != 0) count++;
  return count;
}
...
double ChrCopyNumber::getXiSum(int ploidy, float minSD) {
  ...
  for (int i  = 0; i <= (int)bpfinal_.size(); i++) {
    ...
    if (!((d == 0) && (fragmentNotNA_lengths_[i] == 0))) {
      double sqrtVar = sqrt(float(PI / 2. / fragmentNotNA_lengths_[i])) * max(
        minSD,
        sd_[i]);
  ...
  for (int i  = 0; i <= (int)bpfinal_.size(); i++) {
    ...
    if (level != NA) {
      ...
      if (fragmentNotNA_lengths_[i] != 0) {
        double sqrtVar = sqrt(float(PI / 2. / fragmentNotNA_lengths_[i])) * max(sds.find(
                                                                                  level)->second,
                                                                                sd_[
                                                                                  i]);
        double term = pow(d / sqrtVar, 2);
  ...
  for (int i  = 0; i <= (int)bpfinal_.size(); i++) {
    ...
    if (level != NA) {
      ...
      if ((fragmentNotNA_lengths_[i] != 0) && (sd_[i] != 0)) {
        ...
        // double sqrtVar =
        // sqrt(float(PI/2./fragmentNotNA_lengths_[i]))*max(sds.find(level)->second,sd_[i]);
        double sqrtVar = sqrt(float(PI / 2. / fragmentNotNA_lengths_[i])) *
                         localSV;
        double term = pow(d / sqrtVar, 2);
        sum += term;
      }
...
void ChrCopyNumber::deleteFlanks(int telo_centromeric_flanks) {
  ...
  for (int i = 0; i < (int)medianValues_.size(); i++) {
    if (medianValues_[i] == NA) {
      if ((i - 1 >= 0) &&
          (fragmentNotNA_lengths_[i - 1] <= maxRegionLengthToDelete)) {
        deleteFragment(i - 1);

        for (int j = i - 2; j > 0; j--) {
          if (fragmentNotNA_lengths_[j] <=
              maxRegionLengthToDelete) deleteFragment(j);
          else break;
        }
      }

      if ((i + 1 < (int)medianValues_.size()) &&
          (fragmentNotNA_lengths_[i + 1] <=
           maxRegionLengthToDelete)) deleteFragment(
          i + 1);
    }
  }
  ...
  if (medianValues_[0] != NA)
    if (fragmentNotNA_lengths_[0] <= maxRegionLengthToDelete) deleteFragment(0);

  if ((medianValues_[medianValues_.size() - 1] != NA) &&
      (fragmentNotNA_lengths_[medianValues_.size() - 1] <=
       maxRegionLengthToDelete)) deleteFragment(medianValues_.size() - 1);
}
...
void ChrCopyNumber::recalcFlanks(int telo_centromeric_flanks,
                                 int minNumberOfWindows) {
  int maxRegionLengthToDelete = int(telo_centromeric_flanks / step_);

  for (int i = 0; i < (int)medianValues_.size(); i++) {
    if ((medianValues_[i] == NA) &&
        (fragment_lengths_[i] >= minNumberOfWindows)) {
      int left  = i;
      int right = i;

      if ((i - 1 >= 0) &&
          ((fragmentNotNA_lengths_[i - 1] <= maxRegionLengthToDelete) &&
           (medianValues_[i - 1] != NA))) {
        left = i - 1;

        for (int j = i - 2; j > 0; j--) {
          if ((fragmentNotNA_lengths_[j] <= maxRegionLengthToDelete) &&
              (medianValues_[j] != NA)) left = j;
          else break;
        }
      }
      ...
      if ((i + 1 < (int)medianValues_.size()) &&
        ...
        for (int j = i + 2; j < (int)fragment_lengths_.size() - 1; j++) {
          if ((fragmentNotNA_lengths_[j] <= maxRegionLengthToDelete) &&
              (medianValues_[j] != NA)) right = j;
          else break;
        }
      }

      if (left < i - 1) recalcFlanksForIndeces(left, i - 1);

      if (i + 1 < right) recalcFlanksForIndeces(i + 1, right);
    }
  }
  ...
  if (medianValues_[0] != NA)
    if (fragmentNotNA_lengths_[0] <= maxRegionLengthToDelete) {
      ...
      if ((i + 1 < (int)medianValues_.size()) &&
        ...
        for (int j = i + 2; j < (int)fragment_lengths_.size() - 1; j++) {
          if (fragmentNotNA_lengths_[j] <= maxRegionLengthToDelete) right = j;
          else break;
        }
      }
  ...
  if ((medianValues_[medianValues_.size() - 1] != NA) &&
    ...
    if ((i - 1 >= 0) &&
      ...
      for (int j = i - 2; j > 0; j--) {
        if (fragmentNotNA_lengths_[j] <= maxRegionLengthToDelete) left = j;
        else break;
      }
...
void ChrCopyNumber::recalcFlanksForIndeces(int i_start, int i_end) {
  ...
  for (int i = i_start; i <= i_end; i++) { // collect data points:
    ...
    if (i > 0) breakPointStart = bpfinal_[i - 1] + 1;
    breakPointEnd = bpfinal_[i];
  ...
  if (i_start > 0) breakPointStart = bpfinal_[i_start - 1] + 1;
...
void ChrCopyNumber::createMappabilityProfile() {
  for (int i = 0; i < length_; i++) mappabilityProfile_.push_back(0);
}
...
void ChrCopyNumber::checkOrCreateNotNprofileWithZeros() {
  ...
  for (int i = 0; i < length_; i++) notNprofile_[i] = 0;
}
...
void ChrCopyNumber::fillCGprofile(std::string const& chrFolder) {
  ...
  { filename, filename + ".fa", filename + ".fasta", "chr" + filename + ".fa",
    "chr" + filename + ".fasta" };
  ...
  if (!file.is_open())         {
    //	throw ("Unable to open fasta file for chr "+chromosome_+" in
    // "+chrFolder+"\n");
    cerr << "Unable to open fasta file for chr " + chromosome_ + " in folder " +
      chrFolder + "\n\nPlease note, " << chrFolder <<
      " should be a folder, not a file!\n\n";
    exit(-1);
  }

  // string myString;
  char letter; // here we read the chromosome letter by letter. Can do it
               // better!
  file >> letter;
  int count   = 0;
  int countCG = 0;
  ...
  if (ends_.size() == 0) { // all windows have equal length => can use the same
                           // windowsize for all windows
    for (int i = 0; i < length_; i++) {
      if (file.eof()) {
        GCprofile_[i] = NA;
      ...
      if (count == countN) GCprofile_[i] = NA;
      else GCprofile_[i] = float(countCG) / (count - countN);
    ...
    for (int i = 0; i < length_; i++) {
      ...
      for (int j = 0; j < (int)text.length(); j++) // ++j????
        if ((text[j] == 'C') || (text[j] == 'G') || (text[j] == 'c') ||
            (text[j] == 'g')) countCG++;
        else if (text[j] == 'N') countN++;

      if (end - start + 1 - countN >
          0) GCprofile_[i] = float(countCG) / (end - start + 1 - countN);
      else GCprofile_[i] = NA;
      ...
      if (i + 1 < length_) {
        ...
        if (nextStart <= end) {
          ...
          if (howMuchToDelete < 0) {
            cerr <<
              "Error: your BED file with coordinates of targeted regions does not seem to be sorted\nCheck chromosome "
                 << chromosome_ << "\n";
            cout <<
              "Exit Control-FREEC: before reruning, please, sort the BED file with coordinates of the targeted regions\n";
            exit(-1);
          }
...
float ChrCopyNumber::getEstimatedBAFuncertaintyAtI(int i) {
  if ((i > 0) &&
      (i <
       int(estimatedBAFuncertainty_.size()))) return estimatedBAFuncertainty_[i];
...
void ChrCopyNumber::setLookingForSubclones(bool value) {
  ...
  if (value) {
    if (coordinates_.size() ==
        0) cerr <<
        "Warning: you should intialize the ChrCopyNumber object before calling this function!!!\n";
...
void ChrCopyNumber::createBAF(float value) {
  for (int i = 0; i < length_; i++) BAF_.push_back(value);
}

void ChrCopyNumber::createBAFvalues() {
  for (int i = 0; i < length_; i++) BAFvalues_.push_back("");
}
...
void ChrCopyNumber::addBAFinfo(SNPinGenome& snpingenome, int indexSNP) {
  ...
  for (int i = 0; i < length_; i++) {
    ...
    if ((getSNPpos >= left) && (getSNPpos <= right)) {
      ...
      if ((currentBAFstatus != 0) && (currentBAF != NA)) { // there are values
                                                           // that indicate that
                                                           // this SNP can be
                                                           // heterozygios
        if (BAFvalues_[i] != "") BAFvalues_[i] += ";";
      ...
      if (SNPcount + 1 < totalSNPnumber) { // EV bug fix: 2013-01-17
        ...
        {
          i = max(-1, i - windowSize_ / step_ - 2); // changed -1 to -2 in
                                                    // version 10.9
        }
  ...
  if (ratio_.size() == 0) {
    cerr <<
      "Warning: Normalized read counts (ratio_) has not been initialized; check your parameters\n";
  }

  for (int i = 0; i < length_; i++) {
    if ((ratio_.size() > i) && (ratio_[i] == NA) && (BAF_[i] != NA)) // set
                                                                     // BAF=NA
                                                                     // in
                                                                     // windows
                                                                     // with
...
float ChrCopyNumber::getLevelAt(int unsigned i, int ploidy) {
  ...
  if (value > 0) valueToReturn = float(int(value * ploidy + 0.5)) / ploidy;
  else valueToReturn = float(int(value * ploidy)) / ploidy;

  // use BAF for ambigious cases:
  if ((valueToReturn > 0) && (estBAFuncertaintyPerFrag_.size() > i) &&
      (BAFsymbPerFrag_[i].compare("-") != 0) && (BAFsymbPerFrag_[i] != "") &&
      (BAFsymbPerFrag_[i].length() != valueToReturn * ploidy) &&
      (estBAFuncertaintyPerFrag_[i] < MAXUncertainty)) {
...
{
  ChrCopyNumberCalculateBreakpointArgWrapper *warg =
    (ChrCopyNumberCalculateBreakpointArgWrapper *)arg;
  int result =
    warg->chrCopyNumber.calculateBreakpoints(warg->breakPointThreshold,
                                             0,
                                             warg->breakPointType);

  if (result <= 0) {
    cerr << "..failed to run segmentation on chr" <<
      warg->chrCopyNumber.getChromosome() << "\n";
  }
  return NULL;
}
...
void* ChrCopyNumber_calculateBAFBreakpoint_wrapper(void *arg)
{
  ChrCopyNumberCalculateBreakpointArgWrapper *warg =
    (ChrCopyNumberCalculateBreakpointArgWrapper *)arg;
  int result =
    warg->chrCopyNumber.calculateBAFBreakpoints(warg->breakPointThreshold,
                                                0,
                                                warg->breakPointType);

  if (result == 0) {
    cerr << "..failed to run BAF segmentation on chr" <<
      warg->chrCopyNumber.getChromosome() << "\n";
  }
  return NULL;
}
