              ...
              lrreport               & ar)
{
  ...
  for (j = 0; j <= nvars - 1; j++)
  {
    ...
    lm.w(offs + nvars) = lm.w(offs + nvars) - lm.w(offs + j) * means(j) /
                         sigmas(j);
    v = means(j) / sigmas(j);
    ap::vsub(&ar.c(nvars, 0),
             1,
             &ar.c(j, 0),
             1,
             ap::vlen(0, nvars),
             v);
    ap::vsub(&ar.c(0, nvars),
             ar.c.getstride(),
             &ar.c(0, j),
             ar.c.getstride(),
             ap::vlen(0, nvars),
...
void lrunpack(const linearmodel& lm, ap::real_1d_array& v, int& nvars)
{
  ...
  ap::ap_error::make_assertion(ap::round_f(lm.w(1)) == lrvnum,
                               "LINREG: Incorrect LINREG version!");
                  ...
                  int                      npoints)
{
  ...
  {
    v =
      ap::vdotproduct(&xy(i, 0), 1, &lm.w(offs), 1, ap::vlen(0, nvars - 1));
    v      = v + lm.w(offs + nvars);
...
{
  ap::ap_error::make_assertion(ap::round_f(ra(0)) == lrvnum,
                               "LRUnserialize: incorrect array!");
             ...
             double                 & p)
{
  ...
  if (ap::fp_less_eq(ap::minreal(e1, e2),
                     1000 * ap::machineepsilon *ap::maxreal(e1, e2)))
  {
    info = -3;
    return;
  }
                       ...
                       lrreport               & ar)
{
    ...
    for (k = nvars; k >= 1; k--)
    {
      if (ap::fp_greater(sv(k - 1), epstol * ap::machineepsilon *sv(0)))
      {
        ...
        for (i = 0; i <= npoints - 1; i++)
        {
          ...
          {
            r =
              ap::vdotproduct(&xy(i, 0), 1, &vt(j, 0), 1, ap::vlen(0, nvars - 1));
            xym(i, j) = r;
          }
          xym(i, k) = xy(i, nvars);
        }
  ...
  for (i = 0; i <= nvars - 1; i++)
  {
    ...
    {
      r =
        ap::vdotproduct(&vm(i, 0), 1, &vm(j, 0), 1, ap::vlen(0, nvars - 1));
      ar.c(i, j) = r;
      ar.c(j, i) = r;
    }
  }
