...
int SNPinGenome::processSNPLine(bool    isVCF,
                                char   *line,
                                string& myChr,
                                int   & index,
                                int   & previousPos) {
  ...
  if (strs_cnt >= 3) {
    ...
    if (position != previousPos) {
      ...
      if (int(SNP_atChr_->size()) > index) {
        if (!isVCF) {
          if (strlen(strs[4]) == 1) {
            (*SNP_atChr_)[index].push_SNP(SNPposition(position, strs[2], strs[3],
                                                      strs[4]));
            return 1;
          }
          return 0;
        } else if ((strlen(strs[3]) == 1) && (strlen(strs[4]) < 10)) {
          (*SNP_atChr_)[index].push_SNP(SNPposition(position, strs[4])); // if
                                                                         // VCF
          return 1;
        } else return 0;
      } else {
...
void SNPinGenome::readSNPs(std::string const& inFile)
{
  ...
  std::cout << "PROFILING [tid=" << pthread_self() << "]: " << inFile <<
    " read in " << (time(NULL) - t0) << " seconds [readSNPs]\n" << std::flush;
#endif // ifdef PROFILE_TRACE
}
...
void SNPinGenome::readMateFile(std::string const& mateFile,
  ...
  // must perform partly GenomeCopyNumber::readCopyNumber line #114, all but
  // fillMyHash
...
void SNPinGenome::perform(std::string const& mateFile,
                          ...
                          const char        *what)
{
  ...
  if (genomeCopyNumber.ifHasRatio()) {
    ...
    genomeCopyNumber.calculateCopyNumberMedians(minCNAlength,
                                                noisyData,
                                                CompleteGenomicsData);

    cout << "..Reannotate copy numbers" << std::endl;

    if (genomeCopyNumber.getWESanalysis() ==
        false) genomeCopyNumber.calculateCopyNumberProbs_and_genomeLength(
        breakPointType);
    else genomeCopyNumber.calculateCopyNumberProbs_and_exomeLength(breakPointType);
  }

  cout << "..Done" << std::endl;
}

long SNPinGenome::processPileUPLine(int             & positionCount,
                                    char             *line,
                                    string          & oldChr,
      ...
      if (p_genomeCopyNumber) {
        ...
        if (lindex != NA) {
          if (valueToReturn = strccnt(strs[4], '^')) {
            ChrCopyNumber& chrCopyNumber = p_genomeCopyNumber->getChrCopyNumberAt(
              lindex);
            int step = p_genomeCopyNumber->getStep();

            for (int i = 0; i < valueToReturn;
                 i++) chrCopyNumber.mappedPlusOneAtI(currentPosition, step);
          }
        }
      }
    } else {
      if (p_genomeCopyNumber) {
        ...
        if (lindex != NA) {
          if (valueToReturn = strccnt(strs[4], '^')) {
            ...
            if (((currentPosition - 1 < chrCopyNumber.getEndAtBin(l)) &&
              ...
              while ((!((currentPosition - 1 < chrCopyNumber.getEndAtBin(l)) &&
                        (currentPosition >
                         (chrCopyNumber.getCoordinateAtBin(l)))))  &&
            ...
            if (leftIsInTheWindow == true)  {
              ...
              for (int i = 0; i < valueToReturn;
                   i++) chrCopyNumber.mappedPlusOneAtI(currentPosition, step, l);
            }
          }
        }
      }
  ...
  if (currentPosition == sNPpositionToProceed) {
    try {
      float localBAF = addInfoFromAPileUp(atoi(strs[3]),
                                          minimalTotalLetterCountPerPosition,
                                          (*SNP_atChr_)[index].getNucleotideAt(
                                            positionCount),
                                          index,
                                          positionCount,
                                          sNPpositionToProceed,
                                          strs[4],
                                          minimalQualityPerPosition,
                                          strs[5]);
  ...
  if ((currentPosition > sNPpositionToProceed) && (sNPpositionToProceed != NA)) {
    while (currentPosition > sNPpositionToProceed && sNPpositionToProceed != NA) {
      ...
      if (positionCount <
          (*SNP_atChr_)[index].getSize()) sNPpositionToProceed =
          (*SNP_atChr_)[index].getPositionAt(positionCount);
      else sNPpositionToProceed = NA;
    }
...
void SNPinGenome::readPileUP(FILE             *stream,
                             int               minimalTotalLetterCountPerPosition,
                             int               minimalQualityPerPosition,
                             GenomeCopyNumber *p_genomeCopyNumber) {
  ...
  while ((line_buffer = getLine(buffer, MAX_BUFFER, stream, line)) != NULL) {
    normalCount += processPileUPLine(positionCount,
                                     line_buffer,
                                     oldChr,
                                     sNPpositionToProceed,
                                     minimalTotalLetterCountPerPosition,
...
void SNPinGenome::assignValues(std::string const& inFile,
                               string             inputFormat,
                               int                minimalTotalLetterCountPerPosition,
                               int                minimalQualityPerPosition,
                               GenomeCopyNumber  *p_genomeCopyNumber) {
  ...
  if (((inputFormat.compare("pileup") == 0) ||
    ...
    cout << "..Starting reading " << inFile << " to calculate BAF profiles" <<
      std::endl;
#ifdef PROFILE_TRACE
    time_t t0 = time(NULL);
#endif // ifdef PROFILE_TRACE
    ...
    //        int tmp = calculateBreakpoints_general(-0.0001,
    // heterozygousBAFs05.size(), heterozygousBAFs05, bpfinal, 0, 4);
    ...
    //            float localMedian = get_median(heterozygousBAFs05, start,
    // bpfinal[i]);
    ...
    //        }
    //        std::ofstream file
    // ("/bioinfo/users/vboeva/Desktop/Neuroblastome/ANALYSES_CNG/Valentina_analysis/freec/rowBAFS.txt");
    //        for (int i = 0; i< heterozygousBAFs.size(); i++)
    //            file << heterozygousBAFs[i]<<"\t"<<medianBAFs[i]
    // <<"\t"<<heterozygousBAFs05[i]<<"\t"<<heterozygousBAFposition[i]<<"\n";
    //        file.close();
    ...
    // END TMP:
  } else { // if (inputFormat.compare("eland")==0 ||
...
float SNPinGenome::addInfoFromAPileUp(int         totalLetterCount,
                                      int         minimalTotalLetterCountPerPosition,
                                      char        whatToLook,
                                      int         index,
                                      int       & positionCount,
  ...
  if (totalLetterCount >= minimalTotalLetterCountPerPosition) {
    ...
    if (strccnt(pileup, int('+')) + strccnt(pileup, int('-')) == 0) {
      ...
      if (totalLetterCount >= minimalTotalLetterCountPerPosition) {
        ...
        int countForOverLetter = strccnt(pileup, whatToLook) + strccnt(pileup,
                                                                       tolower(
                                                                         whatToLook));
        ...
        if (refCount + countForOverLetter >= minimalTotalLetterCountPerPosition) {
          if (countForOverLetter + refCount > totalLetterCount) {
            ...
            if (countForOverLetter < 0) countForOverLetter = 0;
          }
          ...
          // "\n";
          // cout << "countForOverLetter-1: "<< countForOverLetter-1 << ";
          // totalLetterCount: "<< totalLetterCount << "; ERROR_PER_POS: "<<
          // ERROR_PER_POS << "\n";
          double prob = binomialcdistribution(countForOverLetter - 1,
                                              totalLetterCount,
                                              ERROR_PER_POS); // probability
                                                              // that there > k
                                                              // Beta Allel
                                                              // bases if the
                                                              // SNP is HomoZ.
          ...
          if (prob < 0.05) {                                  // 0.01
            if (binomialcdistribution(totalLetterCount - countForOverLetter - 1,
                                      totalLetterCount, ERROR_PER_POS) < 0.01)
...
void* SNPinGenome_perform_wrapper(void *arg)
{
  ...
  warg->snpingenome.perform(warg->mateFile,
                            warg->inputFormat,
                            warg->minimalTotalLetterCountPerPosition,
                            warg->minimalQualityPerPosition,
                            warg->noisyData,
                            warg->CompleteGenomicsData,
                            warg->genomeCopyNumber,
                            warg->breakPointThreshold,
                            warg->breakPointType,
                            warg->minCNAlength,
                            warg->what);
  return NULL;
