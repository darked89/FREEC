...
public:
  ...
  void   calculateRatio(ChrCopyNumber control,
                        const double *a,
                        const int     degree);
  void   calculateRatioLog(ChrCopyNumber control,
                           const double *a,
                           const int     degree);
  int    calculateBreakpoints(double breakPointThreshold,
                              int    firstChrLength,
                              int    breakPointType);
  int    calculateBAFBreakpoints(double breakPointThreshold,
                                 int    firstChrLength,
                                 int    breakPointType);
  double calculateXiSum(int ploidy,
                        std::map<float, float>& sds,
                        std::map<float, float>& meds);
  double calculateXiSum(int ploidy,
                        std::map<float, float>& sds);
  void   calculateCopyNumberMedian(); // create median profiles using 'bpfinal_'
                                      // and store them in medianProfile_, info
  ...
  void recalcFlanks(int telo_centromeric_flanks,
                    int minNumberOfWindows); // merge short notNA-segments
                                             // around NA-segments

  // void calculateRatio(double a,double b, double c, double d); //depreciated
  void              calculateRatio(double *a,
                                   int     degree);
  void              recalculateRatio(double *a,
                                     int     degree);

  void              removeLowReadCountWindows(ChrCopyNumber control,
                                              const int     RCThresh);
  ...
  int               getCoveredPart(int breakPointStart,
                                   int breakPointEnd); // for exome-seq: get
  ...
  double            getXiSum(int   ploidy,
                             float minSD); // OLD unUsed
  ...
  float             nextNoNAMedian(int i1,
                                   int ploidy);
  int               nextNoNALength(int i1,
                                   int ploidy);
  int               nextNoNAIndex(int i1,
                                  int ploidy,
                                  int min_fragment);
  ...
  void setNotNprofileAt(int   i,
                        float value);
  void setMappabilityProfileAt(int   i,
                               float value);
  void setBAFat(int   i,
                float value);
  ...
  float getLevelAt(int unsigned i,
                   int          ploidy);

  int   getExons_Countchr();
  void  setCN_subc(int i,
                   int CN_subc);
  int   getCN_subc(int i);
  void  setPopulation_subc(int   i,
                           float pop_subc);
  ...
  float getSmoothedForInterval(int start,
                               int end);
...
private:
  ...
  std::vector<float>notNprofile_;              // percentage of not 'N' in a
                                               // window
  std::vector<float>mappabilityProfile_;       // percentage of mappable
                                               // positions in a window
  std::vector<float>smoothedProfile_;
  std::vector<float>BAF_;                      // median value of abs(BAF-0.5)
                                               // in a window (only for
                                               // heterozygous SNPs)
  std::vector<float>medianBAFProfile_;         // median values of BAF_ for each
                                               // segment)
  std::vector<std::string>BAFvalues_;          // values of abs(BAF-0.5) in a
                                               // window
  std::vector<float>estimatedBAFProfile_;      // estimation of BAF for each
                                               // segment (value per window)
  std::vector<float>fittedBAFProfile_;
  std::vector<std::string>medianBAFSymbol_;    // estimation of BAF for each
                                               // segment: AA,AAB;AB,AABB etc
  std::vector<float>estimatedBAFuncertainty_;  // uncertainty of estimation of
                                               // BAF for each segment
                                               // (1./(LL_best-LL_secondBest)))
  std::vector<std::string>BAFsymbPerFrag_;     // estimation of BAF for each
                                               // segment: AA,AAB;AB,AABB etc -
                                               // one value per fragment
  std::vector<float>estBAFuncertaintyPerFrag_; // uncertainty of estimation of
                                               // BAF for each segment
                                               // (1./(LL_best-LL_secondBest)))
                                               // - one value per fragment
};
#endif // ifndef _CHR_CPN_H
...
struct ChrCopyNumberCalculateBreakpointArgWrapper : public ThreadArg {
  ...
  ChrCopyNumberCalculateBreakpointArgWrapper(ChrCopyNumber& chrCopyNumber,
                                             double         breakPointThreshold,
                                             int            breakPointType) :
    chrCopyNumber(chrCopyNumber), breakPointThreshold(
