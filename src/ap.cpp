...
********************************************************************/
double ap::vdotproduct(const double *v0,
                       int           stride0,
                       const double *v1,
                       int           stride1,
                       int           n)
{
  ...
  if ((stride0 != 1) || (stride1 != 1))
  {
    ...
    // slow general code
    //
    for (i = 0; i < n; i++, v0 += stride0, v1 += stride1) result += (*v0) * (*v1);
  }
  else
  {
    //
    ...
    for (i = 0; i < n4;
         i++, v0 += 4,
         v1 += 4) result += v0[0] * v1[0] + v0[1] * v1[1] + v0[2] * v1[2] +
                            v0[3] *
                            v1[3];
...
ap::complex ap::vdotproduct(const ap::complex *v0,
                            int                stride0,
                            const char        *conj0,
                            const ap::complex *v1,
                            int                stride1,
               ...
               int           n)
{
  ...
  if ((stride_dst != 1) || (stride_src != 1))
  {
    ...
    //
    for (i = 0; i < n; i++, vdst += stride_dst, vsrc += stride_src) *vdst = *vsrc;
  }
  else
  {
    //
...
void ap::vmove(ap::complex       *vdst,
               int                stride_dst,
               const ap::complex *vsrc,
               int                stride_src,
               const char        *conj_src,
               int                n)
{
  ...
  if ((stride_dst != 1) || (stride_src != 1))
  {
    ...
    {
      for (i = 0; i < n;
           i++, vdst += stride_dst, vsrc += stride_src) *vdst = *vsrc;
    }
  }
  else
  {
...
void ap::vmoveneg(ap::complex       *vdst,
                  int                stride_dst,
                  const ap::complex *vsrc,
                  int                stride_src,
                  const char        *conj_src,
               ...
               double        alpha)
{
  ...
  if ((stride_dst != 1) || (stride_src != 1))
  {
    ...
    //
    for (i = 0; i < n;
         i++, vdst += stride_dst, vsrc += stride_src) *vdst = alpha * (*vsrc);
  }
  else
  {
    //
...
double ap::randomreal()
{
  ...
  while (i1 == RAND_MAX) i1 = rand();

  while (i2 == RAND_MAX) i2 = rand();
  double mx = RAND_MAX;
  return (i1 + i2 / mx) / mx;
}
