...
GenomeDensity::GenomeDensity(std::string const& mateFileName,
                             ...
                             std::string const& chrLenFileName)
{
  ...
  if (((inputFormat.compare("eland") == 0) ||
    ...
    while (std::getline(fileMates, line)) {
      ...
      if (strs.size() == 14) {
        ...
        if (((matesOrientation.compare(strs[8] + strs[10]) == 0) &&
             (atoi(strs[13].c_str()) > 0)) ||
            ((matesOrientation.compare(strs[10] + strs[8]) == 0) &&
             (atoi(strs[13].c_str()) < 0))) {
          // cout << line <<"\n";
          string chr = strs[11];
  ...
  } else if (((inputFormat.compare("bowtie") == 0) ||
    ...
    while (std::getline(fileMates, line)) {
      ...
      if (strs.size() > 4) {
        ...
        if ((pos = chr.find("chr", pos)) != string::npos) chr.replace(pos, 3, "");
        else {
          if (chr.find("NC_") == string::npos) {
            int $len = chr.length();
            chr = chr.substr($len - 2);

            if (chr.compare("23")) chr = "X";
            else if (chr.compare("24")) chr = "Y";
            else if (chr.compare("25")) chr = "M";
            else if (chr[0] == '0') chr = chr[1];
          }
        }
        ...
        for (int i = left; i < left + length;
             i++) gd_.find(chr)->second.coveragePlusOneAtI(i);
  ...
  if (count > 0) { // calculation of median and variation for insert size for
                   // mate pair data
    ...
    for (int i = 0; i < numberOfPointsToEvaluateIQR;
         i++) selectedValues[i] = insertSizeVector[i * step];
...
void GenomeDensity::writeToFile(std::string outFileName,
                                std::string type,
                                bool        ifBinary) {
  std::ofstream file;

  if (ifBinary) file.open(outFileName.c_str(), ios::in | ios::binary);
  else file.open(outFileName.c_str());
  ...
  if (type.compare("norm") == 0) {
    ...
    for (it = gd_.begin(); it != gd_.end(); it++) {
      ...
      for (int i = 0; i < (*it).second.getLength(); i++) {
        ...
        if (ifBinary) file.write((char *)&x, sizeof(IntPoint));
        else {
          file << x.chr + sep;
          file << x.pos;
          file << sep;
    ...
    for (it = gd_.begin(); it != gd_.end(); it++) {
      ...
      for (int i = 0; i < (*it).second.getLength(); i++) {
        ...
        if (ifBinary) file.write((char *)&x, sizeof(FloatPoint));
        else {
          file << x.chr + sep;
          file << x.pos;
          file << sep;
...
void GenomeDensity::printDensity(std::string key, int i, int j) {
  if (gd_.find(key) == gd_.end()) return;

  for (int k = i; k < j;
       k++) cout << gd_.find(key)->second.getDensityAtI(k) << "\n";
}
