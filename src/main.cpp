...
static void print_version()
{
  ...
  ostr << "Control-FREEC v" << std::fixed << std::setprecision(1) <<
    FREEC_VERSION <<
...
static const char* get_conf_file(int argc, char *argv[])
{
  ...
  if ((argc > 3) ||
      ((strcmp(argv[1],
               "-conf") != 0) &&
       (strcmp(argv[1], "-config") != 0) && (strcmp(argv[1], "--conf") != 0))) {
    usage();
    exit(0);
  }
  ...
  if (!ifile) {
    std::cerr <<
      "\n\tCould not find your config file.. Please, check the existance of " <<
      conf_file << "\n\n";
    exit(-1);
  }
...
int main(int argc, char *argv[])
{
  ...
  double breakPointThreshold = (double)cf.Value("general",
                                                "breakPointThreshold",
                                                .8);
  ...
  if (breakPointThreshold < 0) {
    ...
    cerr <<
      "Starting from FREEC v.4.2 we use the threshold on the slope of the slope of the RSSs (instead of simply slope in FREEC v.<4.1) to define number of breakpoints in segmentation. ";
    cerr <<
      "This method is more robust and should provide a more uniform segmentation for different chromosomes.\n";
    cerr << "\n\tWe recomend to use \"breakPointThreshold=0.8\"\n\n";
    cerr <<
      "It should be a positive value. The higher it is, the less breakpoints you will get.\n";
    cerr <<
  ...
  std::cout <<
    "..Breakpoint threshold for segmentation of copy number profiles is " <<
    breakPointThreshold << "\n";

  int teloCentroFlanks = (int)cf.Value("general",
                                       "telocentromeric",
                                       TELO_CENTRO_FLANCS);
  ...
  bool contaminationAdjustment = (bool)cf.Value("general",
  ...
  if (knownContamination > 0) {
    if (contaminationAdjustment == false) {
      std::cerr <<
        "..set contaminationAdjustment=TRUE if you want to use \"contamination=...\"\n";
      exit(0);
    }
    ...
    std::cout << "..Contamination by normal cells set to:\t" <<
      knownContamination * 100 << "%\n";
  }
  ...
  if (CompleteGenomicsData) {
    cout <<
      ".. will shift expected BAF values towards zero as you deal with unperfect CompleteGenomics data"
         << endl;
  }

  string pathToSamtools =
    (std::string)cf.Value("general", "samtools", "samtools");
  string pathToBedtools =
    (std::string)cf.Value("general", "bedtools", "bedtools");
  ...
  if (pathToSambamba != "")    {
    ...
    if (SambambaThreads == "")       {
      ...
      cerr <<
        "Warning: the number of thread to use with Sambamba (option \"SambambaThreads\" in [general] has been set to "
           << SambambaThreads << endl;
      cerr <<
        "..in the config file, you can set SambambaThreads = 2 to use 2 threads";
    }
  }
  ...
  bool has_coefficientOfVariation = cf.hasValue("general",
                                                "coefficientOfVariation");
  float coefficientOfVariation = (float)cf.Value("general",
                                                 "coefficientOfVariation",
                                                 0.05);

  if (has_coefficientOfVariation && !has_window) {
    cout << "..Coefficient Of Variation set equal to " <<
      coefficientOfVariation << "\n..it will be used to evaluate window size\n";

    if (coefficientOfVariation <= 0) {
      cerr << "Error: 'coefficientOfVariation' must be positive\n";
      cout <<
        "..Since coefficientOfVariation' must be positive, FREEC will continue running with coefficientOfVariation=0.05\n";
      coefficientOfVariation = 0.05;
    }
  } else if (has_coefficientOfVariation && has_window) {
    cout <<
      "..Note, the Coefficient Of Variation won't be used since \"window\" = " <<
      window << " was set\n";
  } else if (!has_coefficientOfVariation && has_window) {
  ...
  if (step > window) {
    cerr <<
      "Warning: you cannot use a value for the step size larger than window size\nWill set step "
         << window << "\n";
    step = window;
  }
  ...
  if (access(outputDir.c_str(), 0) == 0)    {
    ...
    if (status.st_mode & S_IFDIR)   {
      ...
      cerr << "Error: The path you entered for 'outputDir': " << outputDir <<
        " is a file. It shoud be a directory" << endl;
      exit(-1);
    }
  }  else   {
  ...
  if (has_dirWithFastaSeq) {
    if (access(dirWithFastaSeq.c_str(), 0) == 0)      {
      ...
      if (status.st_mode & S_IFDIR)    {
        cout << "..Directory with files containing chromosome sequences:\t" <<
          dirWithFastaSeq << "\n";
      }
      else   {
        cerr << "Error: The path you entered for 'dirWithFastaSeq': " <<
          dirWithFastaSeq << " is a file. It shoud be a directory" << endl;
        exit(-1);
      }
    }   else    {
      cerr << "Error: Path " << dirWithFastaSeq <<
        " doesn't exist. Comment the line with 'chrFiles' if you use a precalculated GC-content profile or a control sample. Otherwise, set the correct path"
           << endl;
      exit(-1);
    }
  }
  ...
  int minimalTotalLetterCountPerPosition =
    round_f(float(cf.Value("BAF", "minimalCoveragePerPosition", 0)));

  if (minimalTotalLetterCountPerPosition > 0) {
    cout << "..will use a threshold of " << minimalTotalLetterCountPerPosition <<
      " read(s) per SNP position to calculate beta allel frequency (BAF) values\n";
  }

  int minimalQualityPerPosition =
    int(cf.Value("BAF", "minimalQualityPerPosition", 0));
  int shiftInQuality = (int)cf.Value("BAF", "shiftInQuality", 0);

  if (minimalQualityPerPosition > 0) {
    cout << "..will use a quality threshold of " << minimalQualityPerPosition <<
      " to select nucleotides used in calculation of beta allel frequency (BAF) values\n";
    cout << "..will shift qualities by " << shiftInQuality <<
      " when selecting nucleotides used in calculation of beta allel frequency (BAF) values\n";
    cout <<
      "..Note, use shiftInQuality=33 for Sanger or Illumina 1.8+ format; shiftInQuality=64 for Illumina 1.3+\n";
    minimalQualityPerPosition += shiftInQuality;
  }

  bool has_sample_MateFile           = cf.hasValue("sample", "mateFile");
  bool has_sample_mateCopyNumberFile = cf.hasValue("sample",
                                                   "mateCopyNumberFile");
  std::string sample_MateFile           = "";
  std::string sample_MateCopyNumberFile = "";
  string sample_inputFormat             =
    std::string(cf.Value("sample", "inputFormat", ""));
  std::string sample_mateOrientation = (std::string)cf.Value("sample",
                                                             "mateOrientation",
                                                             "0");
  sample_mateOrientation = (std::string)cf.Value("sample",
                                                 "matesOrientation",
                                                 sample_mateOrientation);
  ...
  if (has_sample_MateFile) {
    ...
    if (sample_inputFormat.compare("") == 0) {
      cerr << "Error: You need to set the inputFormat to be avaible to read " <<
        sample_MateFile << "\n";
      cerr <<
        "Available formats:SAM, BAM, pileup, Eland, BED, SOAP, arachne, psl (BLAT) and Bowtie\n";
      cerr <<
        "FREEC accepts 'inputFormat=pileup' and BAM when the user uses option [BAF]\n";
      exit(0);
    } else {
      cout << "..Sample input format:\t" << sample_inputFormat << "\n";
    }

    if ((sample_inputFormat.compare("BAM") == 0) ||
        (sample_inputFormat.compare("bam") == 0) ||
        (sample_inputFormat.compare("Bam") == 0)) {
      if (pathToSambamba != "")
      {
        cout << "..will use this instance of sambamba: '" << pathToSambamba <<
          "' to read BAM files\n";
      }
      else
      {
        cout << "..will use this instance of samtools: '" << pathToSamtools <<
          "' to read BAM files\n";
      }
    }
  }
  ...
  if (has_sample_mateCopyNumberFile) {
    sample_MateCopyNumberFile =
      std::string(cf.Value("sample", "mateCopyNumberFile"));
    cout << "..Sample file with precalculated copy numbers:\t" <<
      sample_MateCopyNumberFile << "\n";
  }

  if (!has_sample_MateFile && !has_sample_mateCopyNumberFile) {
    cerr <<
      "Error: either \"mateFile\" or \"mateCopyNumberFile\" must be specified\n\n";
    exit(0);
  }
  ...
  bool has_control_mateCopyNumberFile = cf.hasValue("control",
                                                    "mateCopyNumberFile");
  std::string control_MateFile           = "";
  std::string control_MateCopyNumberFile = "";
  string control_inputFormat             =
    std::string(cf.Value("control", "inputFormat", ""));
  std::string control_mateOrientation = (std::string)cf.Value("control",
                                                              "mateOrientation",
                                                              "0");
  ...
  if (has_control_MateFile) {
    ...
    if (control_inputFormat.compare("") == 0) {
      ...
      cerr <<
        "Available formats:SAM, BAM, pileup, Eland, BED, SOAP, arachne, psl (BLAT) and Bowtie\n";
      cerr <<
        "FREEC works exclusively with 'inputFormat=pileup' or BAM when the user uses option [BAF]\n";
      exit(0);
    } else {
  ...
  if (has_control_mateCopyNumberFile) {
    control_MateCopyNumberFile =
      std::string(cf.Value("control", "mateCopyNumberFile"));
    cout << "..Control file with precalculated copy numbers:\t" <<
      control_MateCopyNumberFile << "\n";
  }

  bool isControlIsPresent = has_control_MateFile ||
                            has_control_mateCopyNumberFile;
  ...
  bool is_sample_pileup               =
    (sample_inputFormat.compare("pileup") == 0 ||
     sample_inputFormat.compare("SAMtools pileup") == 0);
  bool is_control_pileup =
    (control_inputFormat.compare("pileup") == 0 ||
     control_inputFormat.compare("SAMtools pileup") == 0);

  bool has_BAF           = cf.hasValue("BAF", "SNPfile");
  std::string makePileup = (std::string)cf.Value("BAF",
                                                 "makePileup",
                                                 "false");
  std::string fastaFile = (std::string)cf.Value("BAF",
                                                "fastaFile",
                                                "false");
  std::string miniPileupFileSample = (std::string)cf.Value("sample",
                                                           "miniPileup",
                                                           "false");
  std::string miniPileupFileControl = (std::string)cf.Value("control",
                                                            "miniPileup",
                                                            "false");
  ...
  if ((makePileup != "false") && (fastaFile == "false") &&
      (!isHasMiniPileUPsample || isControlIsPresent && !isHasMiniPileUPcontrol)) {
    cerr <<
      "To create a usable .pileup file from .BAM you need to provide a fasta file for the whole genome with option \"fastaFile\""
         << endl;
    cerr <<
      "If you only want copy number profiles (no genotypes), then remove or comment all the lines in the group of parameters [BAF]"
         << endl;
  ...
  if ((makePileup != "false") &&
      ((!isControlIsPresent) && (!isHasMiniPileUPsample) || isControlIsPresent &&
       (!isHasMiniPileUPcontrol || (miniPileupFileSample == "false"))))
  {
  ...
  if (has_BAF && (makePileup == "false") && !has_sample_MateFile &&
      !isHasMiniPileUPsample) {
    cerr <<
      "ERROR: you need to provide a 'mateFile' for the [sample] (in SAMtools pileup format) to be able to calculate BAF profiles with options [BAF] or to provide a BED/VCF file with SNP positions (option \"makePileup\")\n";
    exit(0);
  }

  if (has_BAF && !has_control_MateFile && isControlIsPresent &&
      (makePileup == "false") && !isHasMiniPileUPcontrol) {
    cerr <<
      "ERROR: you need to provide a 'mateFile' for the [control] (in SAMtools pileup format) to be able to calculate BAF profiles with options [BAF] and detect somatic CNAs and LOH\n";
    cerr <<
      "..Otherwise, you may not to use the control data at all. Just comment or delete 'mateCopyNumberFile' in the [control] group of parameters\n";
    exit(0);
  }
  ...
  if (!is_sample_pileup && has_BAF && (makePileup == "false") &&
    ...
    cout <<
      "..since you mateFile is not in SAMtools pileup format, the BAF values will not be calculated\n";
    has_BAF = false;
  }
  string SNPinfoFile = std::string(cf.Value("BAF", "SNPfile", ""));

  if ((makePileup != "false") && (SNPinfoFile == "") || isHasMiniPileUPsample &&
      (SNPinfoFile == "")) {
    if ((makePileup.substr(makePileup.size() - 3,
                           3) == "vcf") ||
        (makePileup.substr(makePileup.size() - 6, 6) == "vcf.gz")) {
      SNPinfoFile = makePileup;
    } else {
      cerr <<
        "Warning: you have to provide a filename with the \"SNPfile\" option if you wish to calculate BAF profiles\n";
  ...
  //    cerr << "Warning: I would recommend using logLogNorm=TRUE when working
  ...
  std::string GCprofileFile =
    std::string(cf.Value("general", "GCcontentProfile", ""));

  int forceGC =
    int(cf.Value("general", "forceGCcontentNormalization",
                 (ifTargeted && window == 0) ? 1 : 0));
  ...
  if (!normalization) {
    cout <<
      "..FREEC will not normalize data at all; will perform segmentation of read counts in log2(x+1) space.\n";
    logLogNorm = true;
    forceGC    = 0;
    ...
    if (ifTargeted) {
      if (forceGC == 0) {
        isUseGC = false;
        cout <<
          "..FREEC will use only control read counts to normalize copy number profiles.\n";
        cout <<
          "Warning: please, set forceGCcontentNormalization=1 if you want to use GC-content normalization prior to control density normalization for targeted sequencing.\n";
        cout <<
          "..Since version v9.5, forceGCcontentNormalization=1 is default for exome-seq data\n";
      } else {
        cout <<
  ...
  if (isUseGC) {
    cout <<
      "..minimal expected GC-content (general parameter \"minExpectedGC\") was set to "
         << minExpectedGC << "\n";
    cout <<
      "..maximal expected GC-content (general parameter \"maxExpectedGC\") was set to "
         << maxExpectedGC << "\n";
    intercept = (int)(cf.Value("general", "intercept", 1));

    if ((intercept != 1) && !ifTargeted) {
      cout <<
        "Warning: I would advise using 'intercept=1' with your parameters\n";
    }
  } else {
    ...
    if (intercept != 0) {
      cout <<
        "Warning: I would advise using 'intercept=0' with your parameters\n";
    }
  }
  ...
  if (degree != NA) {
    if ((forceGC == 0) ||
        (forceGC ==
         2)) std::cout <<
        "..Polynomial degree for \"ReadCount ~ GC-content\" or \"Sample ReadCount ~ Control ReadCount\" is "
                       <<
        degree << "\n";

    if (forceGC == 1) {
      std::cout << "..Polynomial degree for \"ReadCount ~ GC-content\" is " <<
        degree << "\n";
    ...
    if ((intercept == 1) && !(!has_BAF && isControlIsPresent)) {
      ...
      degree = 1;
      std::cout <<
        "..Polynomial degree for \"Sample ReadCount ~ Control ReadCount\" normalization is "
                << degree << "\n";
    }
  }
  ...
  if (!ifTargeted && logLogNorm && isUseGC && normalization) {
    cerr <<
      "Warning: will not use loglog-normalization since GC-content will be used\n";
    logLogNorm = false;
  }
  ...
  if (targetBed != "" & chrLenFile != "") {
    // make a check that in the chromosome length file there is no chromosomes
    // absent in the captureRegions:
  ...
  minMappabilityPerWindow =
    double(cf.Value("general", "minMappabilityPerWindow", 0.85));

  if (isMinMappabilitySet && isUseGC) {
    cout << "..Using the minimal mappability of: " << minMappabilityPerWindow <<
      "\n";
  } else if (isUseGC) {
    cout << "..Using the default minimal mappability value of " <<
      minMappabilityPerWindow << "\n";
  }
  ...
  bool has_MapFile = cf.hasValue("general",
                                 "gemMappabilityFile");
  string gemMapFile =
    std::string(cf.Value("general", "gemMappabilityFile", ""));
  bool isMappabilityAppliedWithControl = false;

  if (has_MapFile && isControlIsPresent) {
    isMappabilityAppliedWithControl = true;
    cout <<  "..Mappability file" << gemMapFile <<
      " be used: all low mappability positions will be discarded\n";
  }

  if (cf.hasValue("general", "uniqueMatch") && isUseGC) {
    string uMatch = std::string(cf.Value("general", "uniqueMatch"));

    if ((uMatch.compare("1") == 0) || (uMatch.compare("TRUE") == 0) ||
        (uMatch.compare("true") == 0) || (uMatch.compare("True") == 0)) {
      if (!has_MapFile) {
        cout <<
          "Warning: FREEC set 'uniqueMatch=FALSE' since you did not provide a GEM mappability file ('gemMappabilityFile')";
      } else {
        cout << "..Parameter uniqueMatch was set TRUE, will use " << gemMapFile <<
          " for mappability information\n";
        uniqueMatch = true;
      }
    }
  } else if (cf.hasValue("general", "uniqueMatch")) {
    cout <<
      "Warning: FREEC will not use option 'uniqueMatch' since FREEC is not going to use mappability or GC-content for normalization of copy number profiles\n";
  }
  ...
  std::string tryOtherPloidy =
    (std::string)cf.Value("general", "ploidy", "2,3,4");
  ...
  if ((!noisyData) && ifTargeted && has_BAF) {
    cout <<
      "Warning: consider using '[general] noisyData=true' if you expect to have highly nonuniform coverage along the genome\n";
  ...
  if (isControlIsPresent) {
    cout <<
      "..minimal number of reads per window in the control sample is set to " <<
      RCThresh << "\n";
  }

  float seekSubclones =
    (float)cf.Value("general", "minimalSubclonePresence", 100);
  ...
  if ((seekSubclones > 0) &&
      (seekSubclones <
       100)) cout <<
      "..Control-FREEC will look for subclones present in at least " <<
      seekSubclones << "% of cell population\n";
  else cout << "..Control-FREEC will not look for subclones\n";
  ...
  if ((makePileup == "false") && !isHasMiniPileUPcontrol &&
      !isHasMiniPileUPsample) {
    snpingenome.setCopyNumberFromPileup(true); // use pileup for copy number
                                               // assessment, not only for BAFs
  ...
  if ((makePileup != "false") || isHasMiniPileUPcontrol ||
      isHasMiniPileUPsample)  {
    if (!isHasMiniPileUPsample ||
        (isControlIsPresent && !isHasMiniPileUPcontrol)) {
      cout << "Creating Pileup file to compute BAF profile...\n";
      minipileup.makepileup(sampleCopyNumber,
                            controlCopyNumber,
                            sample_MateFile,
                            control_MateFile,
                            myName,
    ...
    cout << "..will use SNP positions from " << SNPinfoFile <<
      " to calculate BAF profiles\n";
    ...
    if (is_sample_pileup && !has_sample_mateCopyNumberFile && has_window)
    {
      ...
      readMateFileArg = new GenomeCopyNumberReadMateFileArgWrapper(snpingenome,
                                                                   samplePileup,
                                                                   "pileup",
    ...
    if (isControlIsPresent)
    {
      ...
      if (is_control_pileup && !has_control_mateCopyNumberFile && has_window)
      {
        ...
        readMateFileArg = new GenomeCopyNumberReadMateFileArgWrapper(
          snpingenomeControl,
          controlPileup,
          "pileup",
          minimalTotalLetterCountPerPosition,
          ...
          step,
          targetBed);
        thrPool->addThread(GenomeCopyNumber_readMateFile_wrapper,
                           readMateFileArg);
  ...
  if (WESanalysis == false)    {
    if (has_sample_mateCopyNumberFile) {
      ...
      if ((step != NA) && (step_fromProfile != step)) {
        cerr << "Error: according to " << sample_MateCopyNumberFile <<
          " your step paramter is equal to step_fromProfile\n";
        cerr << "However, you set step=" << step << " in your config file\n";
        cerr <<
          "This created this error. Either delete \"step\" from your config or set it to the right value of "
             << sample_MateCopyNumberFile << "\n";
        exit(-1);
      }
      ...
      if (!sample_copyNumber_pileup_read && has_window) {
        sampleCopyNumber.readCopyNumber(sample_MateFile,
                                        sample_inputFormat,
                                        sample_mateOrientation,
                                        chrLenFile,
                                        window,
    ...
    if (has_sample_mateCopyNumberFile) {
      ...
      if (!sample_copyNumber_pileup_read) sampleCopyNumber.readCopyNumber(
          sample_MateFile,
          sample_inputFormat,
          sample_mateOrientation,
          chrLenFile,
  ...
  if (isControlIsPresent) {
    if (has_control_mateCopyNumberFile) {
      ...
      if ((WESanalysis == false) && !control_copyNumber_pileup_read) {
        controlCopyNumber.readCopyNumber(control_MateFile,
                                         control_inputFormat,
                                         control_mateOrientation,
                                         chrLenFile,
                                         window,
                                         step);
  ...
  // if it is a TARGETED resequencing experiment, delete all info outside of the
  // target regions
  if (ifTargeted && (WESanalysis == false)) {
    ...
    if (teloCentroFlanks > minRegion) {
      ...
      cout << "..telocenromeric set to " << teloCentroFlanks <<
        " since it is the minimal length of capture regions\n";
    }
  ...
  if (normalization == false) {
    ...
    cout << "Warning: removed " <<
      sampleCopyNumber.removeLargeExons(iqrLargeExon) <<
      "% of exons as too large\n";
    cout <<
      "To avoid it, provide a file with captured targets and not with exons (an exon can be covered by several capture targets)\n";
  }
  ...
  if ((!has_GCprofile || (GCprofileFile == "")) && normalization) {
    if (sampleCopyNumber.getWindowSize() ==
        0) GCprofileFile = outputDir + "GC_profile.targetedRegions.cnp";
    else {
      stringstream ss;
  ...
  if (isUseGC && (WESanalysis == false)) { // then read CG-content.
    ...
    if (has_GCprofile) {                   // a file with CG-content already
                                           // exists
    ...
    } else {                                                             // has_dirWithFastaSeq
                                                                         // is
                                                                         // true
      sampleCopyNumber.fillCGprofile(dirWithFastaSeq);

      if (!has_MapFile) sampleCopyNumber.printCGprofile(GCprofileFile);  // if
                                                                         // has_MapFile
                                                                         // will
                                                                         // print
                                                                         // out
    ...
    if (has_MapFile) {                                                   // read
      ...
      sampleCopyNumber.printCGprofile(GCprofileFile);
      cout << "..Mappability track from " << gemMapFile <<
        " has been added to " << GCprofileFile << "\n";
    }
  } else if (isUseGC && (WESanalysis == true)) { // read the GC for the whole
                                                 // exome:
    cout << "..using GC-content to normalize copy number profiles\n";

    if (has_GCprofile) {                         // a file with CG-content
                                                 // already exists
      cout <<
        "Warning: Will ignore the existing GC-content profile. FREEC will need to recalculate it. You must provide a path to chromosome files, option \"chrFiles\"\n";
    }

    if (has_dirWithFastaSeq) {                                           // has_dirWithFastaSeq
                                                                         // is
                                                                         // true
      sampleCopyNumber.fillCGprofile(dirWithFastaSeq);
      ...
      if (!has_MapFile) sampleCopyNumber.printCGprofile(GCprofileFile);  // if
        ...
        "Error: Cannot read chromosome fasta files for the GC content correction\n";
  ...
  if (isControlIsPresent && isUseGC) {              // && WESanalysis == false
                                                    // removed in v9.4 //then
                                                    // read CG-content and
                                                    // associate it with the
                                                    // control data.
    cout << "..using GC-content to normalize the control profile\n";
    controlCopyNumber.readCGprofile(GCprofileFile); // the file with CG-content
                                                    // already exists

    if (ifTargeted && (WESanalysis == false)) {
      controlCopyNumber.focusOnCapture(targetBed);  // to mask again averything
                                                    // which is not in the
                                                    // capture
      sampleCopyNumber.focusOnCapture(targetBed);   // TODO: Check whether it is
                                                    // needed. can get here only
                                                    // if
  ...
  if (isControlIsPresent) {
    sampleCopyNumber.removeLowReadCountWindows(controlCopyNumber, RCThresh); // remove
                                                                             // window
                                                                             // with
                                                                             // read
                                                                             // count
    ...
    controlCopyNumber.setNormalContamination(0);                             // normal
                                                                             // genome
                                                                             // is
                                                                             // not
                                                                             // contaminated!
    controlCopyNumber.setPloidy(2);                                          // normal
                                                                             // genome
                                                                             // has
                                                                             // ploidy=2!!!
    ...
    if (has_MapFile && isMappabilityAppliedWithControl &&
        (WESanalysis == false)) {
  ...
  if ((seekSubclones < 100) &&
      (seekSubclones > 0)) controlCopyNumber.setSeekSubclones(true);  // CARINO,
                                                                      // WHY DO
                                                                      // YOU
                                                                      // NEED TO
                                                                      // SET IT
  ...
  for (unsigned int i = 0; i < ploidies.size(); i++) {
    ...
    isSuccessfulFit = runWithDefinedPloidy(ploidy,
                                           sampleCopyNumber,
                                           controlCopyNumber,
                                           isControlIsPresent,
                                           forceGC,
                                           ...
                                           seekSubclones,
                                           myName,
                                           unexplainedChromosomes,
                                           CompleteGenomicsData,
                                           normalization);
  }

  cout << "Ploidy" << "\t" << "RSS score" << "\t" <<
    "Percentage of Genome Explained";
  ...
  cout << "..Best ploidy set to " << bestPloidy <<
    " according to the RSS score" << endl;
  int secondBest =
    ploidies[max_element(percentage_GenExpl.begin(),
                         percentage_GenExpl.end()) - percentage_GenExpl.begin()];
  cout << "..Best ploidy could have been set to " << secondBest <<
    " according to the percentage of the copy number changes explained by a model with a given ploidy"
       << endl;

  if ((bestPloidy == 4) &&
      (std::find(ploidies.begin(), ploidies.end(), 2) != ploidies.end())) {
    ...
    if ((percentage_GenExpl[ind4] - percentage_GenExpl[ind2] < 0.05) ||
        (unexplainedChromosomes[ind2] <= 1)) {
      bestPloidy = 2;
      cout <<
        "..Changed ploidy to 2 as there is little difference in the fit betweeen ploidies 4 and 2:"
           << endl;
      cout << "unexplained regions for ploidy 2 are located on " <<
        unexplainedChromosomes[ind2] << " chromosomes" << endl;
    }
  }
  ...
  if (bestPloidy != ploidies.back()) {
    ...
    isSuccessfulFit = runWithDefinedPloidy(bestPloidy,
                                           sampleCopyNumber,
                                           controlCopyNumber,
                                           isControlIsPresent,
                                           forceGC,
  ...
  if (has_BAF || (makePileup != "false") || isHasMiniPileUPsample) {
    ...
    if (ifTargeted) breakPointThreshold_BAF = 1.6;

    if (WESanalysis == true) breakPointThreshold_BAF = 5;
    ...
    {
      snpArg = new SNPinGenomePerformArgWrapper(snpingenome,
                                                sample_MateFile,
                                                sample_inputFormat,
                                                minimalTotalLetterCountPerPosition,
                                                minimalQualityPerPosition,
    ...
    if (isControlIsPresent) {
      if ((makePileup == "false") && !isHasMiniPileUPcontrol)
      {
        snpArg = new SNPinGenomePerformArgWrapper(snpingenomeControl,
                                                  control_MateFile,
                                                  control_inputFormat,
                                                  minimalTotalLetterCountPerPosition,
                                                  minimalQualityPerPosition,
    ...
    if (isControlIsPresent && (makePileup == "false") && !ifTargeted) { // will
                                                                        // output
                                                                        // Control
                                                                        // data
                                                                        // only
                                                                        ...
                                                                        // WGS
                                                                        // data!
      sampleCopyNumber.calculateSomaticCNVs(controlCopyNumber.getCNVs(),
                                            controlCopyNumber.getPloidy());
      controlCopyNumber.printBAF(controlName, snpingenomeControl,
                                 control_MateFile);
      controlCopyNumber.printRatio(myName + "_normal_ratio.txt", 0, printNA);

      if (ifBedGraphOutPut) {
        controlCopyNumber.printRatio(myName + "_normal_ratio.BedGraph", 1,
                                     printNA);
      }
  ...
  if (isUseGC || (forceGC > 0)) file << "CGcontent_Used\tTrue" << endl;

  if (!(isUseGC || (forceGC > 0))) file << "CGcontent_Used\tFalse" << endl;
  file << "Mappability_Used\t" << stringFromBool(sampleCopyNumber.isMappUsed()) <<
    endl;
  file << "Looking_For_Subclones\t" << stringFromBool(isLookingForSubclones) <<
    endl;
  file << "Breakpoint_Threshold\t" << breakPointThreshold << endl;

  // file << "Size_Of_Ignored_Telo_Centromeric_Regions\t"<<teloCentroFlanks<<
  // endl;
  file << "Window\t" << window << endl;
  file << "Number_Of_Reads|Pairs_In_Sample\t" <<
    sampleCopyNumber.getNormalNumberOfPairs() << endl;
  file << "Number_Of_Reads|Pairs_In_Control\t" <<
    controlCopyNumber.getNormalNumberOfPairs() << endl;
  sampleCopyNumber.printInfo(file); // for ploidy & contamination
  file << "Good_Polynomial_Fit\t" << stringFromBool(bool(isSuccessfulFit)) <<
    endl;
...
int runWithDefinedPloidy(int                ploidy,
                         GenomeCopyNumber & sampleCopyNumber,
                         GenomeCopyNumber & controlCopyNumber,
                         bool               isControlIsPresent,
                         int                forceGC,
                         bool               has_BAF,
                         bool               ifTargeted,
                         bool               WESanalysis,
                         ...
                         float              minExpectedGC,
                         float              maxExpectedGC,
                         float              knownContamination,
                         float              breakPointThreshold,
                         int                breakPointType,
                         int                minCNAlength,
                         int                teloCentroFlanks,
                         vector<double>   & RSS,
                         vector<double>   & percentage_GenExpl,
                         bool               contaminationAdjustment,
                         ...
                         float              seekSubclones,
                         std::string        myName,
                         vector<int>      & unexplainedChromosomes,
  ...
  if (normalization) {
    if (isControlIsPresent) {
      if (((!forceGC) && (!has_BAF)) || (ifTargeted && (forceGC != 1)) ||
          ((WESanalysis == true) && (forceGC == 0))) { // normalize sample
                                                       // density with control
                                                       // density
        successfulFit = sampleCopyNumber.calculateRatio(controlCopyNumber,
                                                        degree,
                                                        intercept);
      } else {              // forceGC != 0
        if (forceGC == 1) { // normalize first Sample and Control, and then
                            // calculate the ratio
          if (degree == NA) {
            successfulFit = sampleCopyNumber.calculateRatioUsingCG(intercept,
                                                                   minExpectedGC,
                                                                   maxExpectedGC);

            if (controlCopyNumber.calculateRatioUsingCG(intercept, minExpectedGC,
                                                        maxExpectedGC) ==
                0) successfulFit = 0;
          }
          else {
            successfulFit = sampleCopyNumber.calculateRatioUsingCG(degree,
                                                                   intercept,
                                                                   minExpectedGC,
                                                                   maxExpectedGC);

            if (controlCopyNumber.calculateRatioUsingCG(degree, intercept,
                                                        minExpectedGC,
                                                        maxExpectedGC) ==
                0) successfulFit = 0;
          }
          ...
          successfulFit = sampleCopyNumber.calculateRatio(controlCopyNumber,
                                                          degree,
                                                          intercept);
          sampleCopyNumber.recalculateRatioUsingCG(8,
                                                   1,
                                                   minExpectedGC,
                                                   maxExpectedGC); // try higher
                                                                   // values of
                                                                   // polynomial's
                                                                   // degree
        }
      ...
      if (has_BAF && (forceGC != 1) && !ifTargeted && (WESanalysis == false)) { //
        ...
        if (intercept !=
            1) cerr <<
            "Warning: Again, I would advise using 'intercept = 1' with your parameters\n";
      ...
      if (ifTargeted && (has_BAF) && (forceGC != 1)) {
        cout <<
          "Warning: Control-FREEC will assume that there is not gains and losses in the target regions in the control genome\n";
  ...
  if (knownContamination > 0) {
    cout <<
      "..Recalculating copy number profiles using known value of contamination by normal cells:\n";
  ...
  if ((WESanalysis == true) || (ifTargeted == true))   {
    GenomeCopyNumberCalculateBreakpointArgWrapper *bkpArg =
      new GenomeCopyNumberCalculateBreakpointArgWrapper(sampleCopyNumber,
                                                        breakPointThreshold,
                                                        breakPointType);
    ...
    {
      bkpArg = new GenomeCopyNumberCalculateBreakpointArgWrapper(
        controlCopyNumber,
        breakPointThreshold,
        breakPointType);
      thrPool->addThread(GenomeCopyNumber_calculateBreakpoint_wrapper, bkpArg);
  ...
  cout << std::flush;
  sampleCopyNumber.calculateCopyNumberMedians(minCNAlength,
                                              0,
                                              CompleteGenomicsData);
  ...
  if (contaminationAdjustment && (knownContamination == 0)) {
    ...
    if (contamValue > 0) {
      ...
      // sampleCopyNumber.deleteFlanks(TELO_CENTRO_FLANCS);
      if (WESanalysis ==
          false) sampleCopyNumber.recalcFlanks(teloCentroFlanks, 3);
  ...
  percentage_GenExpl.push_back(sampleCopyNumber.Percentage_GenomeExplained(
                                 unexplainedChromosomesByThisPloidy));
