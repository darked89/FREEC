...
void rmatrixqr(ap::real_2d_array& a, int m, int n, ap::real_1d_array& tau)
{
  ...
  while (blockstart != minmn)
  {
    ...
    ap::vmove(&tau(blockstart), 1, &taubuf(0), 1,
              ap::vlen(blockstart, blockstart + blocksize - 1));
    ...
    {
      if ((n - blockstart - blocksize >= 2 * ablasblocksize(a)) ||
          (rowscount >= 4 * ablasblocksize(a)))
      {
        //
        ...
        // Q' = E + Y*T'*Y' = E + TmpA*TmpT'*TmpA'
        //
        rmatrixgemm(blocksize,
                    n - blockstart - blocksize,
                    rowscount,
                    1.0,
                    tmpa,
                    0,
      ...
      else
      {
        ...
        {
          ap::vmove(&t(1), 1, &tmpa(i, i), tmpa.getstride(),
                    ap::vlen(1, rowscount - i));
          t(1) = 1;
          applyreflectionfromtheleft(a,
                                     taubuf(i),
                                     t,
                                     blockstart + i,
                                     m - 1,
  ...
  while (blockstart != minmn)
  {
    ...
    if (blockstart + blocksize <= m - 1)
    {
      if (m - blockstart - blocksize >= 2 * ablasblocksize(a))
      {
        ...
        // Q  = E + Y*T*Y'  = E + TmpA'*TmpT*TmpA
        //
        rmatrixgemm(m - blockstart - blocksize,
                    blocksize,
                    columnscount,
                    1.0,
                    a,
               ...
               ap::complex_1d_array& tau)
{
  ...
  while (blockstart != minmn)
  {
    ...
    ap::vmove(&tau(blockstart), 1, &taubuf(0), 1, "N",
              ap::vlen(blockstart, blockstart + blocksize - 1));
    ...
    if (blockstart + blocksize <= n - 1)
    {
      ...
      else
      {
        ...
        for (i = 0; i <= blocksize - 1; i++)
        {
          ap::vmove(&t(1), 1, &tmpa(i, i), tmpa.getstride(), "N",
                    ap::vlen(1, rowscount - i));
          t(1) = 1;
          complexapplyreflectionfromtheleft(a,
                                            ap::conj(taubuf(i)),
                                            t,
                                            blockstart + i,
                                            m - 1,
  ...
  while (blockstart != minmn)
  {
    ...
    if (blockstart + blocksize <= m - 1)
    {
      if (m - blockstart - blocksize >= 2 * ablascomplexblocksize(a))
      {
        ...
        // Q  = E + Y*T*Y'  = E + TmpA'*TmpT*TmpA
        //
        cmatrixgemm(m - blockstart - blocksize,
                    blocksize,
                    columnscount,
                    1.0,
                    a,
      ...
      else
      {
        ...
        for (i = 0; i <= blocksize - 1; i++)
        {
          ap::vmove(&t(1),
                    ...
                    1,
                    "Conj",
                    ap::vlen(1, columnscount - i));
          t(1) = 1;
          complexapplyreflectionfromtheright(a,
                                             taubuf(i),
                                             t,
                                             blockstart + blocksize,
                                             m - 1,
                      ...
                      ap::complex_2d_array      & q)
{
  ...
  while (blockstart >= 0)
  {
    ...
    //
    cmatrixcopy(rowscount, blocksize, a, blockstart, blockstart, tmpa, 0, 0);
    ap::vmove(&taubuf(0), 1, &tau(blockstart), 1, "N",
              ap::vlen(0, blocksize - 1));
    ...
    else
    {
      ...
      for (i = blocksize - 1; i >= 0; i--)
      {
        ap::vmove(&t(1), 1, &tmpa(i, i), tmpa.getstride(), "N",
                  ap::vlen(1, rowscount - i));
        t(1) = 1;
        complexapplyreflectionfromtheleft(q,
                                          taubuf(i),
                                          t,
                                          blockstart + i,
                                          m - 1,
                      ...
                      ap::complex_2d_array      & q)
{
  ...
  while (blockstart >= 0)
  {
    ...
    else
    {
      ...
      for (i = blocksize - 1; i >= 0; i--)
      {
        ap::vmove(&t(1), 1, &tmpa(i, i), 1, "Conj",
                  ap::vlen(1, columnscount - i));
        t(1) = 1;
        complexapplyreflectionfromtheright(q,
                                           ap::conj(taubuf(i)),
                                           t,
                                           0,
                                           qrows - 1,
                          ...
                          bool                     dotranspose)
{
  ...
  ap::ap_error::make_assertion((fromtheright && zcolumns == m) ||
  ...
  else
  {
    ...
    if (m - 1 > 0)
    {
      ...
      do
      {
        ap::vmove(&v(1), 1, &qp(i + 1, i), qp.getstride(),
                  ap::vlen(1, m - i - 1));
        v(1) = 1;

        if (fromtheright)
        {
          applyreflectionfromtheright(z,
                                      tauq(i),
                                      v,
                                      0,
                                      zrows - 1,
                          ...
                          bool                     dotranspose)
{
  ...
  ap::ap_error::make_assertion((fromtheright && (zcolumns == n)) ||
...
/*************************************************************************
   Unpacking matrix H (the result of matrix A reduction to upper Hessenberg
     form)
  ...
  if (isupper)
  {
    ...
    for (i = n - 2; i >= 0; i--)
    {
      ...
      if (ap::fp_neq(taui, 0))
      {
        ...
        //
        v =
          ap::vdotproduct(&tau(0), 1, &a(0, i + 1), a.getstride(),
                          ap::vlen(0, i));
        alpha = -0.5 * taui * v;
               ...
               ap::real_1d_array   & e)
{
  ...
  if (isupper)
  {
    ...
    for (i = n - 2; i >= 0; i--)
    {
      ...
      if (taui != 0)
      {
        ...
        //    A := A - v * w' - w * v'
        //
        ap::vmove(&t(1), 1, &a(0, i + 1), a.getstride(), "N",
                  ap::vlen(1, i + 1));
        ap::vmove(&t3(1), 1, &tau(0), 1, "N",
                  ap::vlen(1, i + 1));
  ...
  else
  {
    ...
    for (i = 0; i <= n - 2; i++)
    {
      ...
      //
      ap::vmove(&t(1), 1, &a(i + 1, i), a.getstride(), "N",
                ap::vlen(1, n - i - 1));
      complexgeneratereflection(t, n - i - 1, taui);
      ap::vmove(&a(i + 1, i), a.getstride(), &t(1), 1, "N",
                ap::vlen(i + 1, n - 1));
      e(i) = a(i + 1, i).x;
                      ...
                      ap::complex_2d_array      & q)
{
  ...
  else
  {
    for (i = n - 2; i >= 0; i--)
    {
      ...
      // Apply H(i)
      //
      ap::vmove(&v(1), 1, &a(i + 1, i), a.getstride(), "N",
                ap::vlen(1, n - i - 1));
      v(1) = 1;
      complexapplyreflectionfromtheleft(q, tau(i), v, i + 1, n - 1, 0, n - 1,
                                        work);
    }
  }
}
  ...
  for (k = 0; k <= lengtha - 1; k++)
  {
    for (j = 1; j <= blocksize - 1; j++)
    {
      if (columnwisea)
      {
        ...
        if (ap::fp_neq(v, 0))
        {
          ap::vadd(&t(j, blocksize), 1, &a(k, 0), 1,
                   ap::vlen(blocksize, blocksize + j - 1), v);
        }
      }
      else
      {
        ...
        if (ap::fp_neq(v, 0))
        {
          ap::vadd(&t(j, blocksize), 1, &a(0, k), a.getstride(),
                   ap::vlen(blocksize, blocksize + j - 1), v);
        }
      }
    }
                                  ...
                                  ap::complex_1d_array& work)
{
  ...
  for (k = 0; k <= blocksize - 1; k++)
  {
    ...
    for (i = 0; i <= k - 1; i++)
    {
      ...
      {
        v =
          ap::vdotproduct(&a(i, k), 1, "N", &a(k, k), 1, "Conj",
                          ap::vlen(k, lengtha - 1));
      }
      work(i) = v;
    }
    ...
    {
      v =
        ap::vdotproduct(&t(i, i), 1, "N", &work(i), 1, "N", ap::vlen(i, k - 1));
      t(i, k) = -tau(k) * v;
    }
    t(k, k) = -tau(k);
