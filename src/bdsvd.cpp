...
static bool bidiagonalsvddecompositioninternal(ap::real_1d_array& d,
                                               ...
                                               int                vstart,
                                               int                ncvt);
static double extsignbdsqr(double a,
                           double b);
                  ...
                  int                ncvt)
{
  ...
  result = bidiagonalsvddecompositioninternal(d1,
                                              e1,
                                              n,
                                              isupper,
                                              isfractionalaccuracyrequired,
                                ...
                                int                ncvt)
{
  ...
  result = bidiagonalsvddecompositioninternal(d,
                                              e,
                                              n,
                                              isupper,
                                              isfractionalaccuracyrequired,
                                               ...
                                               int                ncvt)
{
  ...
  if (!isupper)
  {
    ...
    if (nru > 0)
    {
      applyrotationsfromtheright(fwddir,
                                 ustart,
                                 uend,
                                 1 + ustart - 1,
                                 n + ustart - 1,
    ...
    if (ncc > 0)
    {
      applyrotationsfromtheleft(fwddir,
                                1 + cstart - 1,
                                n + cstart - 1,
                                cstart,
                                cend,
  ...
  while (true)
  {
    ...
    if (ll == m - 1)
    {
      ...
      if (ncvt > 0)
      {
        mm0 = m + (vstart - 1);
        mm1 = m - 1 + (vstart - 1);
        ap::vmove(&vttemp(vstart),
                  1,
                  &vt(mm1, vstart),
                  1,
                  ap::vlen(vstart, vend),
                  cosr);
        ap::vadd(&vttemp(vstart),
                 1,
                 &vt(mm0, vstart),
                 1,
                 ap::vlen(vstart, vend),
        ...
        ap::vsub(&vt(mm0, vstart),
                 1,
                 &vt(mm1, vstart),
                 1,
                 ap::vlen(vstart, vend),
                 sinr);
        ap::vmove(&vt(mm1, vstart), 1, &vttemp(vstart), 1,
                  ap::vlen(vstart, vend));
      }
      ...
      if (nru > 0)
      {
        mm0 = m + ustart - 1;
        mm1 = m - 1 + ustart - 1;
        ap::vmove(&utemp(ustart), 1, &u(ustart, mm1), u.getstride(),
                  ap::vlen(ustart, uend), cosl);
        ap::vadd(&utemp(ustart), 1, &u(ustart, mm0), u.getstride(),
                 ap::vlen(ustart, uend), sinl);
        ap::vmul(&u(ustart, mm0), u.getstride(), ap::vlen(ustart, uend), cosl);
        ap::vsub(&u(ustart, mm0),
                 u.getstride(),
                 &u(ustart, mm1),
                 u.getstride(),
                 ap::vlen(ustart, uend),
                 sinl);
        ap::vmove(&u(ustart, mm1), u.getstride(), &utemp(ustart), 1,
                  ap::vlen(ustart, uend));
      }
      ...
      if (ncc > 0)
      {
        mm0 = m + cstart - 1;
        mm1 = m - 1 + cstart - 1;
        ap::vmove(&ctemp(cstart),
                  1,
                  &c(mm1, cstart),
                  1,
                  ap::vlen(cstart, cend),
                  cosl);
        ap::vadd(&ctemp(cstart),
                 1,
                 &c(mm0, cstart),
                 1,
                 ap::vlen(cstart, cend),
    ...
    if (idir == 1)
    {
      ...
      //
      if (ap::fp_less_eq(fabs(e(m - 1)),
                         fabs(tol) * fabs(d(m))) ||
          (ap::fp_less(tol, 0) && ap::fp_less_eq(fabs(e(m - 1)), thresh)))
      {
        e(m - 1) = 0;
        continue;
      }
    ...
    else
    {
      ...
      //
      if (ap::fp_less_eq(fabs(e(ll)),
                         fabs(tol) * fabs(d(ll))) ||
          (ap::fp_less(tol, 0) && ap::fp_less_eq(fabs(e(ll)), thresh)))
      {
        e(ll) = 0;
        continue;
      }
    ...
    //
    if (ap::fp_greater_eq(tol,
                          0) &&
        ap::fp_less_eq(n * tol * (sminl / smax), ap::maxreal(eps, 0.01 * tol)))
    {
      //
    ...
    else
    {
      ...
      if (idir == 1)
      {
        ...
        //
        f = (fabs(d(ll)) - shift) *
            (extsignbdsqr(double(1), d(ll)) + shift / d(ll));
        g = e(ll);
  ...
  for (i = 1; i <= n - 1; i++)
  {
    ...
    if (isub != n + 1 - i)
    {
      ...
      if (ncvt > 0)
      {
        ...
        ap::vmove(&vttemp(vstart),
                  1,
                  &vt(isub + vstart - 1, vstart),
                  1,
                  ap::vlen(vstart, vend));
        ap::vmove(&vt(isub + vstart - 1, vstart),
                  1,
                  &vt(j + vstart - 1, vstart),
                  1,
                  ap::vlen(vstart, vend));
        ap::vmove(&vt(j + vstart - 1, vstart),
                  1,
                  &vttemp(vstart),
                  1,
                  ap::vlen(vstart, vend));
      ...
      {
        j = n + 1 - i;
        ap::vmove(&utemp(ustart), 1,
                  &u(ustart, isub + ustart - 1), u.getstride(),
                  ap::vlen(ustart, uend));
        ap::vmove(&u(ustart, isub + ustart - 1), u.getstride(),
                  &u(ustart, j + ustart - 1),    u.getstride(),
                  ap::vlen(ustart, uend));
      ...
      if (ncc > 0)
      {
        ...
        ap::vmove(&ctemp(cstart),
                  1,
                  &c(isub + cstart - 1, cstart),
                  1,
                  ap::vlen(cstart, cend));
        ap::vmove(&c(isub + cstart - 1, cstart),
                  1,
                  &c(j + cstart - 1, cstart),
                  1,
                  ap::vlen(cstart, cend));
        ap::vmove(&c(j + cstart - 1, cstart),
                  1,
                  &ctemp(cstart),
                  1,
                  ap::vlen(cstart, cend));
                    ...
                    double& csl)
{
  ...
  {
    tsign =
      extsignbdsqr(double(1), csr) * extsignbdsqr(double(1), csl) * extsignbdsqr(
        double(1),
        f);
  }
  ...
  {
    tsign =
      extsignbdsqr(double(1), snr) * extsignbdsqr(double(1), csl) * extsignbdsqr(
        double(1),
        g);
  }
  ...
  {
    tsign =
      extsignbdsqr(double(1), snr) * extsignbdsqr(double(1), snl) * extsignbdsqr(
        double(1),
        h);
  }
  ...
  ssmin =
    extsignbdsqr(ssmin,
                 tsign * extsignbdsqr(double(1), f) * extsignbdsqr(double(1), h));
}
