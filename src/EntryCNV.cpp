...
EntryCNV::EntryCNV(string      chr,
                   ...
                   int         endCoord,
                   int         copyNumber,
                   float       estimatedBAFuncertainty,
                   std::string medianBAFSymbol,
                   bool        hasBAF) {
  chr_                     = chr;
  start_                   = start;
  end_                     = end;
...
std::string EntryCNV::printEntryCNV(float ploidy) {
  ...
  if (copyNumber_ > ploidy) ss << "gain";
  else if (copyNumber_ < ploidy) ss << "loss";
  else ss << "neutral";

  if (isBAFassessed_ !=
      0) ss << "\t" << medianBAFSymbol_ << "\t" << estimatedBAFuncertainty_;

  if ((type_ != "") &&
      (type_.compare("normal") !=
       0)) ss << "\t" << type_ << "\t" << germlinePercent_;

  if (type_.compare("normal") ==
      0) ss << "\t" << "neutral" << "\t" << germlinePercent_;
  return ss.str();
}
...
float EntryCNV::compare(EntryCNV c2,
                        int      overlapPrecision,
                        int    & left,
                        int    & right,
                        float    ploidy,
                        float    controlPloidy) { // return overlap
  if (chr_.compare(c2.getChr()) != 0) return 0;

  if (((copyNumber_ > ploidy) && (c2.getCopyNumber() > controlPloidy)) ||
