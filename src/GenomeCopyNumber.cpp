...
bool GenomeCopyNumber::isMappUsed() {
  return isMappUsed_;
}

void GenomeCopyNumber::readCopyNumber(std::string const& mateFileName,
                                      std::string const& inputFormat,
                                      std::string const& matesOrientation,
                                      std::string const& chrLenFileName,
  ...
  if (((inputFormat.compare("pileup") == 0) ||
       (inputFormat.compare("SAMtools pileup") == 0))) {
    readNumber = getReadNumberFromPileup(mateFileName);
  } else {
    readNumber = getLineNumber(mateFileName,
                               pathToSamtools_,
                               pathToSambamba_,
                               SambambaThreads_);
  }
  ...
  windowSize =
    round_f(float(1. / (coefficientOfVariation * coefficientOfVariation) /
  ...
  GenomeCopyNumber::fillMyHash(mateFileName,
                               inputFormat,
                               matesOrientation,
                               windowSize,
                               windowSize);
...
void GenomeCopyNumber::readCopyNumber(std::string const& mateFileName,
  ...
  if (WESanalysis == false)  {
    if (step == NA) step = windowSize;

    // check if step value is correct:
    if ((step <= 0) || (step > windowSize)) {
      cerr <<
        "step  should be a positive interger value less than or equal to the window size\n";
      exit(-1);
    }
    step_ = step;
  }
...
void GenomeCopyNumber::initCopyNumber(std::string const& chrLenFileName,
                                      int                windowSize,
                                      int                step,
                                      std::string        targetBed) {
...
void GenomeCopyNumber::calculateBreakpoints(double breakPointThreshold,
                                            int    breakPointType) {
  cout << "..Calculating breakpoints, breakPointThreshold = " <<
    breakPointThreshold << "\n";
  ThreadPool *thrPool = ThreadPoolManager::getInstance()->newThreadPool(
...
void GenomeCopyNumber::calculateBAFBreakpoints(double breakPointThreshold,
                                               int    breakPointType) {
  cout << "..Calculating breakpoints for BAF, breakPointThreshold = " <<
    breakPointThreshold << "\n";
#if 1
  ThreadPool *thrPool = ThreadPoolManager::getInstance()->newThreadPool(
  ...
  // calculate breakpoints for the first chromosome and get its length to
  ...
  if (it != chrCopyNumber_.end()) {
    ...
    firstChromLen = it->calculateBAFBreakpoints(breakPointThreshold,
                                                firstChromLen,
                                                breakPointType);
...
void GenomeCopyNumber::fillMyHash(std::string const& mateFileName,
                                  std::string const& inputFormat_str,
                                  std::string const& matesOrientation_str,
                                  int                windowSize,
                                  int                step,
                                  std::string        targetBed) {
  // read mateFileName and calculate copyNumber
  ...
  //	if ( ! in.good()) {
  //		std::cerr << "ERROR: Opening file `" << mateFileName << "'
  // failed.\n";
  //		return EXIT_FAILURE;
  //	}
  //	in.close();
  ...
  if ((inputFormat_str.compare("bam") == 0) ||
      (inputFormat_str.compare("BAM") == 0) ||
      (mateFileName.substr(mateFileName.size() - 4,
                           4).compare(".bam") == 0) ||
      (mateFileName.substr(mateFileName.size() - 3, 3).compare(".gz") == 0)) {
    fileMates.close();
    ...
    if (mateFileName.substr(mateFileName.size() - 3, 3).compare(".gz") != 0) {
      if (pathToSambamba_ != "")
      {
        command = pathToSambamba_ + " view -t " + SambambaThreads_ + " " +
                  mateFileName;
        myInputFormat = "sam"; // will try to use existing sambamba
        cout <<
          "..sambamba should be installed to be able to read BAM files; will use the following command for sambamba: "
             << pathToSambamba_ + " view -t " + SambambaThreads_ + " " +
          mateFileName << "\n";
      }
      else
      {
        ...
        cout <<
          "..samtools should be installed to be able to read BAM files; will use the following command for samtools: "
             << pathToSamtools_ + " view " + mateFileName << "\n";
      }
    }
    else {
    ...
    while ((line_buffer = getLine(buffer, MAX_BUFFER, stream, line)) != NULL) {
      count++;
      normalCount += processRead(inputFormat,
                                 matesOrientation,
                                 line_buffer,
                                 bin,
                                 targetBed,
    ...
    while (std::getline(fileMates, line)) {
      ...
      if (((inputFormat_str.compare("bowtie") == 0) ||
           (inputFormat_str.compare("Bowtie") == 0)) &&
          (matesOrientation_str.compare("0") != 0)) {
        string line2;
        std::getline(fileMates, line2);
        count++;
        normalCount += processReadWithBowtie(inputFormat_str,
                                             matesOrientation_str,
                                             line,
                                             line2);
      } else {
        normalCount += processRead(inputFormat,
...
#ifdef PROFILE_TRACE
  std::cout << "PROFILING [tid=" << pthread_self() << "]: " << mateFileName <<
    " read in " << (time(NULL) - t0) << " seconds [fillMyHash]\n" << std::flush;
#endif // ifdef PROFILE_TRACE
  ...
  if (normalNumberOfPairs_ == 0) {
    ...
    if ((inputFormat_str.compare("SAM") == 0) ||
        (inputFormat_str.compare("sam") == 0) ||
        (inputFormat_str.compare("BAM") == 0) ||
        (inputFormat_str.compare("bam") == 0)) {
...
void GenomeCopyNumber::recalculateRatio(float contamination) {
  ...
  for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++) {
    if ((sex_.compare("XY") == 0) &&
      ...
      // should take into account that normally one has only one copy of X and
      // Y..
      it->recalculateRatioWithContam(contamination, 0.5, isRatioLogged_);
    } else it->recalculateRatioWithContam(contamination, 1, isRatioLogged_);
  }
}

void GenomeCopyNumber::recalculateRatioUsingCG(int   degree,
                                               bool  intercept,
                                               float minExpectedGC,
                                               float maxExpectedGC) {
  if (degree > MAXDEGREE) {
  ...
  if  (intercept) lrbuild(xy, npoints, nvars, info, lm, ar);
  else lrbuildz(xy, npoints, nvars, info, lm, ar);

  if (info != 1) {
    cerr <<
      "Error in the first linear regression (the first guess about parameters), code: "
         << info << "\n";
  }
  ap::real_1d_array v;
  ...
  if  (intercept) a[degree] = v(degree);
  else a[degree] = 0;
  ...
  float rmserror                  = runEM(x,
                                          y,
                                          a,
                                          degree,
                                          maximalNumberOfIterations,
                                          ploidy_,
                                          maximalNumberOfCopies,
                                          intercept,
                                          normalContamination_);
  ...
  if (degree == 3) {
    cout << "Y = " << a[0] << "*x*x*x+" << a[1] << "*x*x+" << a[2] << "*x+" <<
      a[3] << "\n";
  } else if (degree == 2) {
...
int GenomeCopyNumber::calculateRatioUsingCG(bool  intercept,
                                            float minExpectedGC,
                                            float maxExpectedGC) { // returns 1
                                                                   // if the
                                                                   // number of
                                                                   // iteration
                                                                   // is less
    ...
    for (int i = 0; i < npoints; i++) {
      around[i] = minExpectedGC + (maxExpectedGC - minExpectedGC) /
                  (npoints - 1) * i; // 0.55-0.35
                                     ...
                                     // 0.40, 0.45, 0.5, 0.55 }; for 0.35 and
                                     // 0.55
  ...
  if (realNumberOfIterations < maximalNumberOfIterations) valueToReturn = 1;
  return valueToReturn;
}

int GenomeCopyNumber::calculateRatioUsingCG(int   degree,
                                            bool  intercept,
                                            float minExpectedGC,
                                            float maxExpectedGC) {
  if (degree > MAXDEGREE) {
...
double GenomeCopyNumber::calculateMedianRatioAround(float interval,
  ...
  if (myValuesAround.size() == 0) {
    cerr << "Error: zero reads in windows with the GC-content around " <<
      around << "\n";
...
double GenomeCopyNumber::calculateMedianAround(float interval, float around) {
  ...
  if (myValuesAround.size() == 0) {
    cerr << "Error: zero reads in windows with the GC-content around " <<
      around << " with interval " << interval << ", will try again with " <<
      interval * 4 << "\n";
    interval = interval * 4;
...
double GenomeCopyNumber::calculateMedianAround(
    ...
    if (!((it->getChromosome().find("X") != string::npos) ||
      ...
      for (int i = 0; i < it->getLength(); i++) {
        ...
        if ((controlValue <= maxVal) &&
            (controlValue >= minVal)) myValuesAround.push_back(it->getValueAt(i));
      }
      controlcounts.clear();
    }
  }
  ...
  if (myValuesAround.size() == 0) {
    cerr << "Warning: zero reads in windows with the Read Count around " <<
      around << " in the Contol dataset\n";
    cerr <<
      "May be your window is too small? which value of the window size do you obtain when you set coefficientOfVariation=0.05? \n";
...
void GenomeCopyNumber::removeLowReadCountWindows(
  GenomeCopyNumber& controlCopyNumber,
  int               RCThresh) {
  cout << "..will remove all windows with read count in the control less than " <<
    RCThresh << "\n";
  vector<ChrCopyNumber>::iterator it;
...
void GenomeCopyNumber::removeLowReadCountWindowsFromControl(int RCThresh) {
  cout <<
    "..will process the control file as well: removing all windows with read count in the control less than "
       << RCThresh << "\n";
  vector<ChrCopyNumber>::iterator it;
...
long double GenomeCopyNumber::calculateRSS(int ploidy)
{
  ...
  for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
    ...
    if ((pos = chrNumber.find("X")) != string::npos) // exclude X and Y from the
                                                     // analysis
      continue;

    if ((pos = chrNumber.find("Y")) != string::npos) continue;
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (observed != NA) {
        ...
        if (chrCopyNumber_[index].isMedianCalculated()) {
          ...
          if (chrCopyNumber_[index].isSmoothed()) expected =
              chrCopyNumber_[index].getSmoothedProfileAtI(i);
        }
  ...
  for (int i = 0; i < (int)observedvalues.size(); i++)
  {
    ...
    {
      long double diff = (long double)observedvalues[i] - (long double)round(
        ploidy * expectedvalues[i]) / ploidy;
...
void GenomeCopyNumber::calculateRatioUsingCG(GenomeCopyNumber& controlCopyNumber)
{
  ...
  for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++) {
    ...
    } else it->recalculateRatio(controlCopyNumber.getChrCopyNumber(it->
                                                                   getChromosome()));
  }
}

void GenomeCopyNumber::calculateRatioUsingCG_Regression(
  GenomeCopyNumber& controlCopyNumber) {
  int  degree    = 1;
  bool intercept = 0;
  ...
  if (degree == 1) {
    const char *nametmp =
      "/home/vboeva/NAS_public/data/projects/FREEC/TEST_FREEC_CLBMA_WES/output_WES/xy.txt";
    std::ofstream file;
    file.open(nametmp);
...
int GenomeCopyNumber::calculateRatio(GenomeCopyNumber& controlCopyNumber,
                                     int               degree,
                                     bool              intercept) {
  int maximalNumberOfIterations = 300;
  int maximalNumberOfCopies     = ploidy_ * 2;
  ...
  if (isRatioLogged_) {
    ...
    if (intercept) a[degree] = v(degree);  // intercept
    ...
    for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++) {
      if ((sex_.compare("XY") == 0) &&
        ...
        // should take into account that normally one has only one copy of X and
        // Y..
        it->calculateRatioLog(controlCopyNumber.getChrCopyNumber(
                                it->getChromosome()),
                              a,
                              degree);
        it->recalculateRatio(2);
    ...
    int medianReadCountPerWindowForControl =
      controlCopyNumber.calculateMedianReadCountPerWindow();
    int sdReadCountPerWindowForControl =
      controlCopyNumber.calculateSDReadCountPerWindow(
        medianReadCountPerWindowForControl);
    ...
    // instead of {
    // meanReadCountPerWindowForControl,meanReadCountPerWindowForControl-2*sqrt(double(meanReadCountPerWindowForControl)),meanReadCountPerWindowForControl+2*sqrt(double(meanReadCountPerWindowForControl))
    // };
    ...
    for (int i = 0; i < npoints; i = i + npoints - 1) {
      double value =
        calculateMedianAround(controlCopyNumber, interval, float(around[i]));

      if (value == NA) valuesAreAllRight = 0;
    }

    if (!valuesAreAllRight) {
      // we need to set other values of max and min..
      minVal = max(
        medianReadCountPerWindowForControl - sdReadCountPerWindowForControl,
    ...
    for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++) {
      if ((sex_.compare("XY") == 0) &&
        ...
        // should take into account that normally one has only one copy of X and
        // Y..
        it->calculateRatio(controlCopyNumber.getChrCopyNumber(
                             it->getChromosome()),
                           a,
                           degree);
        it->recalculateRatio(2);
...
float GenomeCopyNumber::calculateNormalizationConstant(
  GenomeCopyNumber& controlCopyNumber) {
...
void GenomeCopyNumber::printRatio(std::string const& outFile,
                                  bool               ifBedGraphOutPut,
                                  bool               printNA) {
...
void GenomeCopyNumber::printBAF(std::string const& outFile,
                                SNPinGenome      & snpingenome,
                                std::string const& matefile) {
  ...
  for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
    ...
    if (indexSNP == NA) {
      cerr <<
        "An error occurred in GenomeCopyNumber::printBAF: could not find an SNP index for "
           << chrNumber << "\n";
      exit(-1);
    }
...
void GenomeCopyNumber::recalcFlanks(int telo_centromeric_flanks,
...
void GenomeCopyNumber::calculateCopyNumberProbs_and_exomeLength(int breakPointType)
{
  ...
  if (hasBAF_)
  {
    ...
    NormalBAF_XY = getXYBAFforPloidy(ploidy_); // will only use it when the
                                               // genome is male
  }
  ...
  for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++)
  {
    ...
    for (int i = 0; i < (int)it->getMedianValues().size(); i++) {
      ...
      if (level == NA)
      {
        ...
        if (nextIndex == NA) {
          ...
          if (hasBAF_) BAFnext = it->getBAFsymbPerFrg(nextIndex);
        }
        ...
        if (breakPointType == NOCALL) {
          if ((previousLevel != NA) && (nextLevel != NA) &&
              (previousLevel == nextLevel) && (previousLevel == normalLevel)) {
            if (BAFprev.compare(BAFnext) == 0) {
              level = previousLevel;
            }
          }
        } else {
          if ((breakPointType == LARGECLOSE) || (breakPointType == SIMPLERIGHT) ||
              (breakPointType == HALFLENGTH)) {
            if ((previousLevel != NA) && (nextLevel != NA)) {
            ...
            if ((previousLevel == normalLevel) || (nextLevel == normalLevel)) {
              ...
              if (BAFprev.compare(BAFnext) == 0) BAFSym = BAFnext;
              else BAFSym = "-";
            } else {
              if ((previousLevel != NA) && (nextLevel != NA)) {
                if (lengthOfPreviousLevel > lengthOfNextLevel) {
          ...
          if (level == NA) level = normalLevel;  // should never happen
        }
      }
      ...
      if (it->getEndsSize() == 0) {
        ...
        if (cnumber == NA) {
          ...
          if ((round_f(level * ploidy_) != cnumber) ||
            ...
            int realEndOfTheCNV = (end + 1) * windowSize_; // check that CNV is
                                                           // not larger than
                                                           // chr size

            if (realEndOfTheCNV >
                it->getChrLength()) realEndOfTheCNV = it->getChrLength();

            if (hasBAF_ && (lBAF != "") &&
                (lBAF.compare("-") != 0)) cnumber = lBAF.length();

            if ((sex_.compare("XY") == 0) &&
                ((it->getChromosome().find("X") != string::npos) ||
                 (it->getChromosome().find("Y") != string::npos))) {
              if ((cnumber != normalXYploidy) && (cnumber != NA)) // push
                                                                  // previous
                                                                  // entry
                if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start,
                                                      end, start * windowSize_,
                                              ...
                                              start * windowSize_,
                                              realEndOfTheCNV, cnumber));  // save
                                                                           // previous
                                                                           // CNV
              else if (hasBAF_ && (lBAF.compare(NormalBAF_XY) != 0) &&
                       (cnumber == normalXYploidy) && (lBAF != "") &&
                       (lBAF.compare("-") != 0))                           // abnormal
                                                                           // BAF
                CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
                                         start * windowSize_, realEndOfTheCNV,
                                         cnumber, lUncertainty, lBAF,
                                         hasBAF_));        // save previous CNV
            } else {
              if ((cnumber != ploidy_) && (cnumber != NA)) // push previous
                                                           // entry
                if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start,
                                                      end, start * windowSize_,
                                                      realEndOfTheCNV, cnumber,
            ...
            for (int j = start; j <= end;
                 j++) it->pushSmoothedProfile(float(cnumber) / ploidy_);
            ...
            end = end + fragmentLength; // should almost never happen if
                                        //  (hasBAF_)
          }
        }
      } else {
        if (cnumber == NA) {
          ...
          if ((round_f(level * ploidy_) != cnumber) ||
            ...
            int realEndOfTheCNV = it->getEndAtBin(end); // check that CNV is not
                                                        // larger than chr size
            ...
            if ((sex_.compare("XY") == 0) &&
                ((it->getChromosome().find("X") != string::npos) ||
                 (it->getChromosome().find("Y") != string::npos))) {
              if ((cnumber != normalXYploidy) && (cnumber >= 0)) // push
                                                                 // previous
                                                                 // entry
                if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start,
                                                      end,
                ...
                CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
                                         it->getCoordinateAtBin(start),
                                         realEndOfTheCNV, cnumber,
                                         lUncertainty, lBAF, hasBAF_));  // save
                                                                         // previous
                                                                         // CNV
            } else {
              if ((cnumber != ploidy_) && (cnumber >= 0))                // push
                                                                         // previous
                                                                         // entry
                if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start,
                                                      end,
    ...
    if (it->getEndsSize() == 0) {
      int realEndOfTheCNV = (end + 1) * windowSize_; // check that CNV is not
                                                     // larger than chr size
      ...
      if ((sex_.compare("XY") == 0) &&
          ((it->getChromosome().find("X") != string::npos) ||
           (it->getChromosome().find("Y") != string::npos))) {
        if ((cnumber != normalXYploidy) && (cnumber != NA)) CNVs_.push_back(EntryCNV(
                                                                              it->
                                                                              getChromosome(),
                                                                              start,
                                                                              end,
        ...
        if ((cnumber != normalXYploidy) && (cnumber != NA))
          if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
                                        ...
                                        start * windowSize_, realEndOfTheCNV,
                                        cnumber));          // save previous CNV
        else if (hasBAF_ && (lBAF.compare(NormalBAF_XY) != 0) &&
                 (cnumber == normalXYploidy) && (lBAF != "") &&
                 (lBAF.compare("-") != 0))                  // abnormal BAF
          CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
      ...
      for (int j = start; j <= end; j++) it->pushSmoothedProfile(
          float(cnumber) / ploidy_);
    } else {
      ...
      if ((sex_.compare("XY") == 0) &&
        ...
        if ((cnumber != normalXYploidy) && (cnumber != NA))
          ...
          CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
                                   ...
                                   cnumber, lUncertainty,
                                   lBAF, hasBAF_));  // save previous CNV
      } else {
        if ((cnumber != ploidy_) && (cnumber != NA))
          if (hasBAF_) CNVs_.push_back(EntryCNV(it->getChromosome(), start, end,
                                                it->getCoordinateAtBin(start),
  ...
  for (it = chrCopyNumber_.begin(); it != chrCopyNumber_.end(); it++) {
    ...
    for (int i = 0; i < (int)it->getMedianValues().size(); i++) {
      ...
      if ((breakPointType == HALFLENGTH) &&
          (i < (int)it->getMedianValues().size() - 1)) {
...
void GenomeCopyNumber::readGemMappabilityFile(std::string const& inFile) {
  ...
  if (file.is_open())     {
    ...
    while (!file.eof())   {
      ...
      if ((line.substr(0, 1) == "~") && (line.substr(0, 2) != "~~")) {
        ...
        if (index == NA) {
          ...
          cout << "..Control: Last window: from " <<
            chrCopyNumber_[index].getCoordinateAtBin(maxInd) << " to " <<
            chrCopyNumber_[index].getEndAtBin(maxInd) << "\n";
        ...
        if (count > endPos) {
          ...
          if (howMuchToDelete < 0) howMuchToDelete = 0;  // this should never
                                                         // happen
          text    = text.substr(howMuchToDelete);
          lastEnd = endPos;
          ...
          for (int i = 0; i < (int)substr_.length(); ++i)
            if (substr_[i] == '!') uniqueCount++;


          if (positionInd + 1 < chrCopyNumber_[index].getLength()) {
            int nextStart = chrCopyNumber_[index].getCoordinateAtBin(
              positionInd + 1);
            ...
            if (nextStart <= endPos) {
              ...
              text = text.substr(localWindowSize); // actually this should be
                                                   // the same as
                                                   // "text.substr(howMuchToDelete);"
            }
          }
          ...
          if (positionInd < chrCopyNumber_[index].getLength()) {
            startPos =
              chrCopyNumber_[index].getCoordinateAtBin(positionInd);
            endPos          = chrCopyNumber_[index].getEndAtBin(positionInd);
            localWindowSize = endPos - startPos + 1;
          } else {
    ...
    cout << "..Mappability profile was taken into account for " <<
      countChromosomes << " chromosomes\n";
    cout << ".." << countChromosomesOutOfIndex << " chromosomes were skipped\n";

    //		if (chrCopyNumber_[1].getLength() ==
    // chrCopyNumber_[1].getMappabilityLength())
  ...
  if (file.is_open())     {
    while (!file.eof())   {
      ...
      if (strs.size() >= 4) {
        ...
        if (observedStep == 0) observedStep = ceil(strtod(strs[1].c_str(), NULL));
        float CGperc          = (float)strtod(strs[2].c_str(), NULL);
        float nonNperc        = (float)strtod(strs[3].c_str(), NULL);
        string::size_type pos = 0;

        if ((pos =
               currentChr.find("chr", pos)) != string::npos) currentChr.replace(
            pos,
            3,
            "");
        ...
        if (index != NA) {
          ...
          if (strs.size() == 5) { // means that there are also mappability
                                  // values in the 5th colomn
            float MappPerc = (float)strtod(strs[4].c_str(), NULL);
            chrCopyNumber_[index].addToMappabilityProfile(MappPerc);
          }

          if ((observedStep != 0) && (observedStep != step_) && (step_ != NA) &&
              (step_ != 0)) {
            file.close();
    ...
    if (count == 0) {
      cerr << "Your GC-content file " << inFile <<
        " is empty or is in a wrong format\n\nPlease use chomosome sequences (option \"chrFiles\") to recreate it!\n\n";
      exit(-1);
    }
  } else {
...
void GenomeCopyNumber::readCopyNumber(std::string const& inFile) {
  if (WESanalysis == false)
  {
    ...
    if (file.is_open())     {
      while (!file.eof())   {
        ...
        if (strs.size() == 3) {
          ...
          if (windowSize_ == 0) windowSize_ = atoi(strs[1].c_str());
          count++;
        }
        else if (strs.size() == 4) {
          ...
          if ((pos =
                 chrNumber.find("chr", pos)) != string::npos) chrNumber.replace(
              pos,
              3,
              "");
          ...
          if ((count > 0) && (step_ == NA)) {
            ...
            step_ = chrCopyNumber_[chrCount].getCoordinateAtBin(1) -
                    chrCopyNumber_[chrCount].getCoordinateAtBin(0);
          }
      ...
      for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
        ...
        if (chrCopyNumber_[(*it).second].getEndsSize() >
            0) chromosomeLength = chrCopyNumber_[(*it).second].getEndAtBin(
            chrCopyNumber_[(*it).second].getEndsSize() - 1);
        ...
        // cout <<(*it).first <<"\t" <<(*it).second << "\t" <<windowSize_ <<
        // "\t" << length << "\t" <<length*windowSize_ << "\n";
      }
    } else {
      cerr << "Unable to open file " + inFile + "\n";
      exit(-1);
    ...
    if (WESanalysis ==
        false) cout << "\t evaluated genome size:\t" << refGenomeSize_ << "\n";
  } else {
    totalNumberOfPairs_  = 0;
    normalNumberOfPairs_ = 0;
    windowSize_          = 0;
    ...
    if (file.is_open())     {
      while (!file.eof())   {
        ...
        if ((pos = chrNumber.find("chr", pos)) != string::npos) chrNumber.replace(
            pos,
            3,
            "");
        ...
        if (strs.size() >= 4) chrCopyNumber_[chrCount].addToGenes_name(
            strs[4].c_str());
        normalNumberOfPairs_ += (int)cp;
...
void GenomeCopyNumber::printRatio(std::string const& chr,
...
void GenomeCopyNumber::printCopyNumber(std::string const& chr,
                                       std::string const& outFile) {
  std::ofstream file(outFile.c_str());
    ...
    if (CNVs_[i].getCopyNumber() > -1) {
      if ((sex_.compare("XY") == 0) &&
          ((CNVs_[i].getChr().find("X") != string::npos) ||
           (CNVs_[i].getChr().find("Y") != string::npos))) {
...
void GenomeCopyNumber::calculateSomaticCNVs(std::vector<EntryCNV>controlCNVs,
                                            int                  controlPloidy) {
  ifUsedControl_ = true;
  ...
  for (int i = 0; i < (int)CNVs_.size(); i++) {
    ...
    for (int unsigned j = 0; j < controlCNVs.size(); j++) {
      ...
      if ((sex_.compare("XY") == 0) &&
          ((CNVs_[i].getChr().find("X") != string::npos) ||
           (CNVs_[i].getChr().find("Y") != string::npos))) {
        overlap = CNVs_[i].compare(controlCNVs[j],
                                   overlapPrecision,
                                   left,
                                   right,
                                   ploidy_ * 0.5,
    ...
    if ((type == "somatic") && (lefts.size() > 0)) {
      ...
      CNVs_[i].setGermlinePercent(germlineLength * 1. /
                                  (CNVs_[i].getEnd() - CNVs_[i].getStart() + 1));
      lefts.clear();
      rights.clear();
...
void GenomeCopyNumber::printRatioBedGraph(std::string const& chr,
                                          std::ofstream    & file,
                                          std::string const& typeCNA) {
  string::size_type pos = 0;
  float  value;
  int    position;
  string myType;
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (chrCopyNumber_[index].isSmoothed()) valueToPrint =
        chrCopyNumber_[index].getSmoothedProfileAtI(i) * ploidy_;
    else valueToPrint =
        round_by_ploidy(chrCopyNumber_[index].getMedianProfileAtI(i),
                        ploidy_) * ploidy_;

    if (valueToPrint < 0) valueToPrint = NA;
    ...
    if (hasBAF_ && (valueToPrint == ploidy_) &&
        (chrCopyNumber_[index].getBAFProfileAt(i) == 0)) {
      myType = "LOH";
    }

    if ((myType.compare(typeCNA) == 0) &&
        (value !=
         NA)) file << "chr" << chrNumber << "\t" << position << "\t" << position +
        windowSize_ << "\t" << value * ploidy_ << "\n";
  }
}
...
void GenomeCopyNumber::printRatio(std::string const& chr,
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (printNA || (ratioToPrint != NA)) { // process this this window
      file << chrNumber << "\t" << chrCopyNumber_[index].getCoordinateAtBin(i) +
        1 << "\t" << ratioToPrint;
      ...
      if (chrCopyNumber_[index].isMedianCalculated()) {
        ...
        if (chrCopyNumber_[index].isSmoothed()) valueToPrint =
            chrCopyNumber_[index].getSmoothedProfileAtI(i) * ploidy_;
        else valueToPrint = round_by_ploidy(chrCopyNumber_[index].getMedianProfileAtI(
                                              i),
                                            ploidy_) * ploidy_;
        ...
        if (hasBAF_) {
          if ((valueToPrint > 0) &&
              (chrCopyNumber_[index].getBAFsymbolAt(i).compare("-") != 0) &&
              (chrCopyNumber_[index].getBAFsymbolAt(i) != "") &&
              (chrCopyNumber_[index].getBAFsymbolAt(i).length() !=
               valueToPrint)) { // &&
                                // (chrCopyNumber_[index].getEstimatedBAFuncertaintyAtI(i)<
                                // MAXUncertainty)
      ...
      if ((WESanalysis == true) &&
          (chrCopyNumber_[index].getGeneNameAtBin(i) != ""))   {
        file <<  "\t" << chrCopyNumber_[index].getGeneNameAtBin(i);
      }

      if (SeekingSubc_ == true)  {
        file << "\t" << chrCopyNumber_[index].getCN_subc(i) << "\t" <<
          chrCopyNumber_[index].getPopulation_subc(i); // check that it is still
                                                       // there
      }
...
void GenomeCopyNumber::printBAF(std::string const& chr,
  ...
  for (int i = 0; i < length; i++) {
    if ((snpAtChrom.getStatusAt(i) != 0) && (snpAtChrom.getValueAt(i) >= 0)) {
      ...
      if (WinNumber != NA) Avalue = chrCopyNumber_[index].getBAFProfileAt(
          WinNumber);
      ...
      if (WinNumber != NA) Afitted = chrCopyNumber_[index].getFittedBAFProfileAt(
          WinNumber);
      float Bfitted = 1 - Afitted;

      if (Afitted == NA) Bfitted = NA;
      file << Afitted << "\t" << Bfitted << "\t" << Avalue << "\t" << Bvalue <<
        "\t" << chrCopyNumber_[index].getEstimatedBAFuncertaintyAtI(WinNumber) <<
        "\n";
    }
  }
...
/*
   int GenomeCopyNumber::findWinNumber(int position, std::string myName,
      std::string const& matefile)
   {
    string line;
    string pileup;
 ...
 */
void GenomeCopyNumber::printCGprofile(std::string const& chr,
                                      std::ofstream    & file) {
  string::size_type pos = 0;
  string chrNumber      = chr;
  ...
  if (index == NA) return; int length = chrCopyNumber_[index].getLength();
...
void GenomeCopyNumber::calculateCopyNumberMedians(int  minCNAlength,
                                                  bool noisyData,
...
void GenomeCopyNumber::calculatePloidy(int minCNAlength) {
  ...
  if (chrCopyNumber_[0].getMedianValues().size() == 0) calculateCopyNumberMedians(
      minCNAlength,
      0,
      0);
...
void GenomeCopyNumber::calculateSDAndMed(int ploidy, map<float, float>& sds,
                                         map<float, float>& meds) {
  float value, med;
  vector<ChrCopyNumber>::iterator it;
...
float GenomeCopyNumber::calculateVarianceForNormalCopy(int ploidy) { // geting
                                                                     // the
                                                                     // total
                                                                     // variance
                                                                     // for
...
void GenomeCopyNumber::printPloidy(std::ofstream& file) {
  file << "Sample ploidy is " << ploidy_ << " with p-value " << ploidy_pvalue_ <<
    std::endl;
}
...
float GenomeCopyNumber::evaluateContamination() {
  ...
  for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
    ...
    if (index == NA) {
      cerr <<
        "An error occurred in GenomeCopyNumber::evaluateContamination: could not find an index for "
           << chrNumber << "\n";
      return 0;
    }
    int length = chrCopyNumber_[index].getLength();
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (observed != NA) {
        if (isRatioLogged_) observed = pow(2, observed);
        float expected = observed;
        ...
        if (chrCopyNumber_[index].isMedianCalculated()) {
          ...
          if (chrCopyNumber_[index].isSmoothed() &&
              (WESanalysis ==
               false)) expected = chrCopyNumber_[index].getSmoothedProfileAtI(i);
        }

        if (!((expected == 1) || (expected <= 0) ||
              (expected >= 1 + 2.0 / ploidy_) || (observed > 3) ||
              (observed <= 0))
            && (((1 > observed) && (1 > expected)) ||
                ((1 < observed) && (1 < expected)))) { // should it be something
                                                       // related to ploidy_ and
                                                       // not 2
          float p = (observed - expected) /
                    (observed - expected + 2 / ploidy_ * (1 - observed));
  ...
  if (contam < 0) {
    cout <<
      "\t..Evaluation of contamination using contamination = (observedLevel-expectedLevel)/(1-expectedLevel) produced a negative value: "
         << contam << "\n";
    cout <<
      "\t..This probably means that there is not contamination at all or your \"ploidy\" value is not correct\n Will continue with contamination = 0\n";
    contam = 0;
  }
...
float GenomeCopyNumber::evaluateContaminationwithLR() {
  ...
  for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (observed != NA) {
        ...
        if (chrCopyNumber_[index].isMedianCalculated()) {
          expected = round_by_ploidy(chrCopyNumber_[index].getMedianProfileAtI(i),
                                     ploidy_);
        }
  ...
  //  float b = (sum_x2 * sum_y - sum_x * sum_xy) / (n * sum_x2 - sum_x *
  // sum_x);
  ...
  if (contam_a < 0) {
    cout << "\t..Evaluation of contamination produced a negative value: " <<
      contam_a << "\n";
    cout <<
      "\t..This probably means that there is not contamination at all or your \"ploidy\" value is not correct\n Will continue with contamination = 0\n";
    contam_a = 0;
  ...
  if (contam_a > 1) {
    cout << "\t..Evaluation of contamination produced a value over 1: " <<
      contam_a << "\n";
    cout <<
      "\t..This probably means that there is not contamination at all or your \"ploidy\" value is not correct\n Will continue with contamination = 0\n";
    contam_a = 0;
...
void GenomeCopyNumber::addBAFinfo(SNPinGenome& snpingenome) {
  ...
  for (it = chromosomesInd_.begin(); it != chromosomesInd_.end(); it++) {
    ...
    if (index == NA) {
      cerr <<
        "An error occurred in GenomeCopyNumber::addBAFinfo: could not find an index for "
           << chrNumber << "\n";
      exit(-1);
    }
    int indexSNP = snpingenome.findIndex(chrNumber);
    ...
    if (indexSNP == NA) {
      cerr <<
        "An error occurred in GenomeCopyNumber::addBAFinfo: could not find an SNP index for "
           << chrNumber << "\n";
      exit(-1);
    }
    ...
    //		int SNPcount = 0;
    //		float currentBAF =
    // snpingenome.SNP_atChr(indexSNP).getValueAt(SNPcount);
    //		int getSNPpos =
...
/*
   int GenomeCopyNumber::processRead(std::string const& inputFormat, std::string
      const& matesOrientation,std::string const line) {
    if (! line.length()) return 0;
    int valueToReturn = 0;

        if ((inputFormat.compare("sam")==0 || inputFormat.compare("SAM")==0
           )&&(matesOrientation.compare("0")!=0)){
        ...
        if (getSAMinfo(line.c_str(),chr1,chr2,orient1,orient2,left,right)) {
                  ...
                  if ((!strcmp(matesOrientation.c_str(), orient1_2) &&
                     (right-left>0)) || (!strcmp(matesOrientation.c_str(),
                     orient2_1) && (right-left<0))) {
    ...
    int left, right,insertSize;
        ...
        if ((inputFormat.compare("eland")==0 ||
           inputFormat.compare("Eland")==0)&&(matesOrientation.compare("0")!=0))
           {
            if
               (getELANDinfo(line,chr1,chr2,orient1,orient2,left,right,insertSize))
               {
                if ((matesOrientation.compare(orient1+orient2)==0 &&
                   insertSize>0) ||
                   (matesOrientation.compare(orient1+orient2)==0 &&
                   insertSize<0)) {
                                        int ind = findIndex(chr1);
                                        if (ind!=NA) {
                        chrCopyNumber_[ind].mappedPlusOneAtI(min(left,right),
                           step_);
                        valueToReturn= 1;
                    }
                                }
        ...
        if ((inputFormat.compare("bowtie")==0 ||
           inputFormat.compare("Bowtie")==0)&&(matesOrientation.compare("0")==0)){
        ...
        if ((inputFormat.compare("psl")==0 ||
           inputFormat.compare("BLAT")==0)&&(matesOrientation.compare("0")==0)){
        ...
        if ((inputFormat.compare("SOAP")==0 || inputFormat.compare("soap")==0 ||
           inputFormat.compare("Soap")==0)&&(matesOrientation.compare("0")==0)){
        ...
        if ((inputFormat.compare("pileup")==0 || inputFormat.compare("SAMtools
           pileup")==0)){
                        if (line[0] == '#') return 0;
                        if ( line[0] == '@') return 0;
   ...
   // we will simply count "^"
    ...
    cerr << "Format of the input file ( "<<inputFormat<<" )was not
       recognized..\n";
    exit (-1);
   }
 */
int GenomeCopyNumber::processRead(InputFormat     inputFormat,
                                  MateOrientation matesOrientation,
                                  ...
                                  std::string     mateFileName)
{
  ...
  if ((inputFormat == SAM_INPUT_FORMAT) &&
      (matesOrientation != SINGLE_END_SORTED_SAM)) {
    ...
    if (WESanalysis == true)  {
      ...
      if (getSAMinfo(line_buffer, chr1, chr2, orient1, orient2, left, right,
        ...
        if (((matesOrientation == orient1_2) && (right - left > 0)) ||
            ((matesOrientation == orient2_1) && (right - left < 0))) {
          left  = min(left, right);
          ...
          if ((index != NA) && (chrCopyNumber_[index].getExons_Countchr() != 0)) {
            ...
            for (l = prevInd - 1;
                 ...
                 l++) {
              if (((left - 1 < chrCopyNumber_[index].getEndAtBin(l)) &&
                   (left >
                    (chrCopyNumber_[index].getCoordinateAtBin(l) -
                     insert_size)))) {
            ...
            if (!leftIsInTheWindow) {
              ...
              for (l = 0;
                   left >=
                   chrCopyNumber_[index].getCoordinateAtBin(l) - 30 *
                   insert_size &&
                   l < chrCopyNumber_[index].getExons_Countchr(); l++) {
                if (((left - 1 < chrCopyNumber_[index].getEndAtBin(l)) &&
                     (left >
                      (chrCopyNumber_[index].getCoordinateAtBin(l) -
                       insert_size)))) {
            ...
            if ((right >  chrCopyNumber_[index].getCoordinateAtBin(l)) &&
              ...
              step_ = chrCopyNumber_[index].getEndAtBin(l) -
                      chrCopyNumber_[index].getCoordinateAtBin(l) + insert_size +
                      1;
  ...
  if ((inputFormat == SAM_INPUT_FORMAT) &&
    ...
    if (strs_cnt > 3) {
      if (WESanalysis == false) {
        ...
        // chr = "chr" + chr; //I do not understand why it was written here.
        // Valentina
        int l = 0;
        ...
        if (chr.compare("*") != 0) {
          ...
          if (index != NA) {
            ...
            if (((left - 1 < chrCopyNumber_[index].getEndAtBin(l)) &&
                 (left >
                  (chrCopyNumber_[index].getCoordinateAtBin(l) - read_Size))))   {
              leftIsInTheWindow = true;
            }   else   {
  ...
  if ((inputFormat == ELAND_INPUT_FORMAT) &&
      (matesOrientation != SINGLE_END_SORTED_SAM)) {
    string chr1, chr2, orient1, orient2;
    int    left, right, insertSize;

    if (getELANDinfo(line_buffer, chr1, chr2, orient1, orient2, left, right,
                     insertSize)) {
      MateOrientation orient1_2 = getMateOrientation(orient1 + orient2);
      MateOrientation orient2_1 = getMateOrientation(orient2 + orient1);

      if (((matesOrientation == orient1_2) && (insertSize > 0)) ||
          ((matesOrientation == orient2_1) && (insertSize < 0))) {
        // EV: warning orient2_1 not used: port "as is" from 5.9 version, bug ?
        // - it was a bug! Valentina
        int ind = findIndex(chr1);
  ...
  if ((inputFormat == BOWTIE_INPUT_FORMAT) &&
      (matesOrientation == SINGLE_END_SORTED_SAM)) {
  ...
  if ((inputFormat == PSL_INPUT_FORMAT) &&
      (matesOrientation == SINGLE_END_SORTED_SAM)) {
    std::vector<std::string> strs = split(line_buffer, '\t');
  ...
  if ((inputFormat == ARACHNE_BED_INPUT_FORMAT) &&
      (matesOrientation == SINGLE_END_SORTED_SAM)) {
    std::vector<std::string> strs = split(line_buffer, '\t');
  ...
  if ((inputFormat == SOAP_INPUT_FORMAT) &&
      (matesOrientation == SINGLE_END_SORTED_SAM)) {
    std::vector<std::string> strs = split(line_buffer, '\t');
  ...
  cerr << "Format of the input file ( " << inputFormat <<
    " )was not recognized..\n";
  exit(-1);
}

int GenomeCopyNumber::processReadWithBowtie(std::string const& inputFormat,
                                            std::string const& matesOrientation,
  ...
  if (((inputFormat.compare("bowtie") == 0) ||
    ...
    if ((strs.size() > 4) && (strs2.size() > 4)) {
      ...
      if (chr1.compare(chr2) == 0) {
        ...
        if (orient1.compare("+") == 0) orient1 = "F";
        else orient1 = "R";

        if (orient2.compare("+") == 0) orient2 = "F";
        else orient2 = "R";
        int left  = atoi(strs[3].c_str());
        ...
        if (((matesOrientation.compare(orient1 + orient2) == 0) &&
             (right - left > 0)) ||
            ((matesOrientation.compare(orient2 + orient1) == 0) &&
             (right - left < 0))) {
          int index = findIndex(chr1);
...
int GenomeCopyNumber::focusOnCapture(std::string const& captureFile) {
  ...
  if (file.is_open())     {
    ...
    while (!file.eof())   {
      ...
      string chr = strs[0];
      processChrName(chr);
      int positionS = atoi(strs[1].c_str()) - averageReadLength;
      ...
      if (rightWindow < leftWindow) rightWindow = leftWindow;
      ...
      for (int i = leftWindow; i <= rightWindow; i++) {
        ...
        if (positionE <
            i * step_ +
            windowSize_) mapCount += (positionE - (i * step_ + windowSize_));
...
void GenomeCopyNumber::setSambamba(std::string const& pathToSambamba,
                                   std::string const& SambambaThreads) {
...
{
  GenomeCopyNumberReadMateFileArgWrapper *warg =
    (GenomeCopyNumberReadMateFileArgWrapper *)arg;

  if (warg->p_genomeCopyNumber) {
    warg->snpInGenome.readMateFile(warg->mateFile,
                                   warg->inputFormat,
                                   warg->minimalTotalLetterCountPerPosition,
                                   warg->minimalQualityPerPosition,
                                   *warg->p_genomeCopyNumber,
                                   warg->chrLenFileName,
                                   warg->windowSize,
                                   warg->step,
                                   warg->targetBed);
...
{
  GenomeCopyNumberCalculateBreakpointArgWrapper *warg =
    (GenomeCopyNumberCalculateBreakpointArgWrapper *)arg;

  warg->genomeCopyNumber.calculateBreakpoints(warg->breakPointThreshold,
                                              warg->breakPointType);
  return NULL;
}
...
double GenomeCopyNumber::Percentage_GenomeExplained(int& unexplainedChromosomes)
{
    ...
    if (!((it->getChromosome().find("X") != string::npos) ||
      ...
      for (int i = 0; i < it->getNumberOfFragments(); i++) { // for each
        ...
        if (fragmentLength > threshold) {
          ...
          if ((fragment_median2 != NA) &&
              (abs(fragment_median2 -
                   round_by_ploidy(fragment_median2,
                                   ploidy_)) >= 1.0 / 3 / ploidy_)) {
  ...
  cout << "Total proportion of unexplained regions: " << sum_frags <<
    " out of " << numberOfPoints << " = " << sum_frags / numberOfPoints << endl;
