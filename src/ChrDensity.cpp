...
void ChrDensity::smooth(KernelVector kernelVector) {
  ...
  for (int i = r - 1; i < length_ - r + 1; i++) {
    ...
    for (int j = i - r + 1; j < i + r;
         j++) sumKY += kernelVector.getK(j, i) * coverage_[j];
    density_[i] = sumKY / sumK;

    if (i % 1000000 ==
        0) cout << i << "\t" << density_[i] << "\t" << coverage_[i] << "\n";
  }
}
...
void ChrDensity::calculateLogRatio(ChrDensity controlCD, std::string type) {
  ...
  if (length_ != controlCD.getLength()) return;  // throw error TODO
  ...
  if (type.compare("norm") == 0) {
    for (int i = 0; i < length_; i++) {
      ...
      if (y != 0) logRatio[i] = float(coverage_[i]) / y;
      else logRatio[i] = -1;

      if (i % 50000 == 0) {
        //	std::cout << i << "\t" << logRatio[i] << "\t" << coverage_[i] <<
        // "\t" << y << "\n";
        std::cout << i << "\t" << exp(-(float)y) *
          pow((float)y,
              coverage_[i]) / factorial(coverage_[i]) << "\t" << -log(exp(
                                                                        (float)y) *
                                                                      pow(
                                                                        (float)y,
                                                                        coverage_
    ...
    for (int i = 0; i < length_; i++) {
      ...
      if (y != 0) logRatio[i] = float(density_[i]) / y;
      else logRatio[i] = -1;

      // if (i % 50000 == 0)
      //	std::cout << i << "\t" << logRatio[i] << "\t" << density_[i] <<
      // "\t" << y << "\n";
    }
  }
}
