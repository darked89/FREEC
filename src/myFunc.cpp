...
# include <limits.h>
#else // if defined(_WIN32) || (defined(__APPLE__) && defined(__MACH__))

// other platforms
# include <values.h>
#endif // if defined(_WIN32) || (defined(__APPLE__) && defined(__MACH__))
...
bool checkChrLen(const std::string& chrLenFile, const std::string& targetBed) {
  ...
  for (int i = 0; i < names.size(); i++) {
    if (std::find(names_bed.begin(), names_bed.end(),
                  names[i]) != names_bed.end()) {
...
float get_sum(const std::vector<float>& data) {
  ...
  for (int i = 0; i < arrayLength; i++) sum += data[i];
  return (float)sum;
}
...
float get_median(const std::vector<float>& myvector) {
  ...
  // Get median
  float median_value = ndatapoints % 2 ==
                       1 ? data[(ndatapoints - 1) /
                                2] : (data[ndatapoints / 2 - 1] +
                                      data[ndatapoints / 2]) /
                       float(2.0);
  data.clear();
  return median_value;
...
int argmin(const std::vector<double>& myvector) {
  ...
  for (int i = 1; i < (int)myvector.size(); i++)
    if (myvector[i] < myvector[argmin]) argmin = i;
...
float get_median(const std::vector<float>& myvector, int start, int end) {
  if ((start >= 0) && (start < end) && (end <= (int)myvector.size())) {
    ...
    for (int i = start; i < end; i++) data[i - start] = myvector[i];
    sort(data.begin(), data.end());
...
// ---------------------------------------------------------------------------
float get_weighted_mean(const std::vector<float>& data,
  ...
  if (arrayLength != weights.size()) {
    cerr <<
      "Warning: arrays should have the same size in \"get_weighted_mean\"..\n";
    return NA;
  }
...
void readChrNamesInBed(const std::string       & targetBed,
                       std::vector<std::string>& names_bed) {
  ...
  while (std::getline(file, line)) {
    ...
    if ((pos = name.find("chr", pos)) != string::npos) name.replace(pos, 3, "");
...
void readFileWithGenomeInfo(const std::string       & chrLenFileName,
                            std::vector<std::string>& names,
                            std::vector<int>        & lengths) {
  ...
  if (chrLenFileName.substr(chrLenFileName.length() - 3,
                            3).compare("fai") == 0) isFai = 1;
...
long getLineNumber(std::string const& fileName,
                   const std::string& pathToSamtools,
                   const std::string& pathToSambamba,
                   const std::string& SambambaThreads) {
  string line;
  long   count = 0;
  FILE  *stream;
...
void advance_to(const std::string& haystack, size_t& offset, char needle) {
  // I think std::string should behave correctly in this case, but just to be
  // sure...
  if (offset >= haystack.size()) return;

  offset = haystack.find(needle, offset);

  if (offset == std::string::npos) offset = haystack.size();
}
...
#ifdef PROFILE_TRACE
  std::cout << "PROFILING [tid=" << pthread_self() << "]: " << fileName <<
    " read in " << (time(NULL) - t0) << " seconds [getReadNumberFromPileup]\n";
#endif // ifdef PROFILE_TRACE
  return count;
}
...
// This function returns the index of the maximum value in the vector 'data'.
// When there
...
// This function adds the constant value x to the elements of the vector a, a0i
// = ai + x.
...
void findnextbreakpoint(const std::vector<float>& x,
                        int                     & besti,
                        int                     & bestsign,
                        float                   & bestlambda,
                        int                       segLength,
  ...
  if (segLength > 2) {
    ...
    if (a == b) {
      ...
      for (int i = 0; i < segLength - 2; i++) f2[i] = (float)i + 1;
...
static void calculateBreakpoints_perform(float **V,
                                         int   **jump,
                                         float **J,
                                         int     from,
                                         int     to,
                                         int     kk)
{
  // std::cout << "calculateBreakpoints_perform from=" << from << " to=" << to
  // << " kk=" << kk << std::endl;
...
int calculateBreakpoints_general(double               threshold,
                                 int                  length,
                                 const vector<float>& ratio,
                                 vector<int>        & bpfinal,
                                 int                  normal_length_dummy,
                                 int                  breakPointType,
  ...
  for (int i = 0; i < length; i++) {
    if (ratio[i] != NA) {
      ...
      if (isLogRat) {
        if (ratio[i] < maxValue) Y.push_back(log(ratio[i]));
        else Y.push_back(log((ratio[i] - maxValue) / (absoluteMax - maxValue) +
                             maxValue));
      } else {
        if (ratio[i] < maxValue) Y.push_back(ratio[i]);
        else Y.push_back(
            (ratio[i] - maxValue) / (absoluteMax - maxValue) + maxValue);
      }
      shift_bp.push_back(shift[i]);
    } else {
      if (i != 0) shift[i] = shift[i - 1] + 1;
      else shift[i] = 1;
    }
  }
  int n = Y.size();
  ...
  if (n < 5) {
    ThreadPoolManager::getInstance()->unlock();
    cerr << "You have " << n <<
      " windows with reads. It is not normal. Please check you parameters\nIf it happens for chrY, maybe your sample is female? Then use sex=XX\nWill try to continue anyway..\n";
    return -1;
  }
#if 0
  double adjust = 1;
  ...
  for (int i = 4; i < n - 4; i++) diffs.push_back(fabs(Y[i + 1] - Y[i - 1]));
  ...
  if (diffs.size() > 10) {
    ...
    if (maxD > 0)
      for (int i = 10; i < n - 10; i++)
        if ((fabs(Y[i + 1] - Y[i - 1]) >= maxD) &&
            (fabs(Y[i + 2] - Y[i - 2]) >= maxD) &&
            (fabs(Y[i + 3] - Y[i - 3]) >= maxD) &&
            (fabs(Y[i + 4] - Y[i - 4]) >= maxD)) bp_vale.push_back(i);
  }
  diffs.clear();
  ...
  for (int i = 1; i < n; i++) u[i] = u[i - 1] + Y[i];
  ...
  for (int i = 0; i < row; i++) stack[i] = new float[col];
  ...
  while (nb < k)  {
    ...
    if (besti > 0)  {
      ...
      // what follows in the "if" loop is an inefficient way to put the first
      // entry into the
      ...
      if (stackcounter == 1)  {
        ...
        while (bestlambda <= lambdas[ac] && lambdas[ac] != 0 &&
               arrows[ac] != 0)      {
          previousarrow = ac;
          ac            = arrows[ac];
          currentarrow  = ac;
        ...
        // ...if it came in last:
        else if (arrows[ac] == 0) arrows[ac] = stackcounter;
        else {
    ...
    if (besti > 0)  {
      ...
      if (stackcounter == 1) {
        ...
        while (bestlambda <= lambdas[ac3] && lambdas[ac3] != 0 &&
               arrows[ac3] != 0) {
          previousarrow = ac3;
          ac3           = arrows[ac3];
          currentarrow  = ac3;
        ...
        // ...if it came in last:
        else if (arrows[ac3] == 0) arrows[ac3] = stackcounter;
        else {
          // .. or if it came in in the middle:
          arrows[previousarrow] = stackcounter;
          arrows[stackcounter]  = currentarrow;
    ...
    bp[nb] = stack[4][currentstackcolumn]; // '='?�: conversion de 'float' en
                                           // 'int', perte possible de donn?�es
    nb++;
  }
  ...
  for (int i = 0; i < row; i++) delete[] stack[i];
  delete[] stack;
  ...
  // bp_vale
  for (int i = 0; i < (int)bp_vale.size(); i++) bp.push_back(bp_vale[i]);

  // cout << bp.size() << " vs k= " <<k <<"\n";
  sort(bp.begin(), bp.end());

  for (int i = bp.size() - 1; i > 0; i--)
    if (bp[i] == bp[i - 1]) bp.erase(bp.begin() + i);
  ...
  for (int i = 0; i < row; i++) J[i] = new float[col];
  ...
  for (int i = 2; i < n + 2; i++) v[i] = v[i - 1] + Y[i - 2] * Y[i - 2];
  ...
  for (int i = 0; i < row; i++) jump[i] = new int[col];

  float **V;
  row = k + 2; col = k + 1;
  V   = new float *[row];

  for (int i = 0; i < row; i++) V[i] = new float[col];

  for (int i = 1; i <= k; i++) V[1][i] = J[1][i];

#ifdef PROFILE_TRACE
  std::cout << "PROFILING [tid=" << pthread_self() <<
    "]: breakpoint prologue computed in " << (time(NULL) - t0) << " seconds\n" <<
    std::flush;
#endif // ifdef PROFILE_TRACE
...
#ifdef PROFILE_TRACE
  std::cout << "PROFILING [tid=" << pthread_self() <<
    "]: main breakpoint loop computed in " << (time(NULL) - t0) << " seconds\n" <<
    std::flush;
#endif // ifdef PROFILE_TRACE

  for (int i = 0; i < k + 1; i++) delete[] J[i];
  delete[] J;
  ...
  for (int i = 0; i < k + 2; i++) delete[] V[i];
  delete[] V;
  ...
  // normalise:
  // float L0 = Ltemp[1]; //*adjust to keep less breakpoints for short
  // chromosomes
  // for (int i=1; i<=k; i++) {
  ...
  // find the largest index at which the gradgradL is larger than some
  // pre-defined cutoff:
  float imax = 0;
  ...
  // keep going until either we get to the end of L, or we get a slope greater
  // than
  ...
  if (rk) {
    ...
    for (int i = rk - 1; i > 0; i--) rjumps[i - 1] = jump[i][rjumps[i] - 1];
  }
  ...
  for (int i = 0; i < k + 1; i++) delete[] jump[i];
  delete[] jump;
  ...
  if (breakPointType == SIMPLERIGHT) {
  ...
  else if ((breakPointType == LARGECLOSE) || (breakPointType == NORMALLEVEL) ||
    ...
    for (int i = 0; i < (int)bpfinal.size(); i++) {
      if (bpfinal[i] != -1) {
        ...
        if (shift_bp[bpfinal[i]] != shift_bp[bpfinal[i] + 1]) { // add a second
                                                                // breakpoint
    ...
    // sort(bpWithAddedPoints.begin(), bpWithAddedPoints.end()); //they should
    // be already sorted...
...
#ifdef PROFILE_TRACE
  std::cout << "PROFILING [tid=" << pthread_self() <<
    "]: breakpoint epilogue computed in " << (time(NULL) - t0) << " seconds\n" <<
    std::flush;
#endif // ifdef PROFILE_TRACE
  ThreadPoolManager::getInstance()->unlock();
...
float round_by_ploidy(float value, int ploidy) {
  if (value > 0) return float(int(value * ploidy + 0.5)) / ploidy;
...
float polynomial(const float  x,
                 const double a0,
                 const double a1,
                 const double a2,
                 const double a3) { // a0x^3+a1x^2+a2x+a3
  return float(a0 * x * x * x + a1 * x * x + a2 * x + a3);
}

float runEM_linear(const vector<float>& x,
                   const vector<float>& y,
                   double             & a0,
                   double             & a1,
                   int                  maximalNumberOfIterations,
                   int                  ploidy,
                   int                  maximalNumberOfCopies) {
  float rmserror = -1;
  ...
  for (count = 0; count < maximalNumberOfIterations; count++) {
    for (int i = 0; i < (int)x.size(); i++) {
      for (int j = 0; j <= maximalNumberOfCopies; j++) res[j] = fabs(
          j * a0 / ploidy * x[i] + j * a1 / ploidy - y[i]);
      cluster[i] = get_min_index(res);
    }
    int npoints = 0;

    for (int i = 0; i < (int)x.size(); i++)
      if (cluster[i] == ploidy) npoints++;
...
float runEM(const vector<float>& x,
            const vector<float>& y,
            double             & a0,
            double             & a1,
            double             & a2,
  ...
  for (count = 0; count < maximalNumberOfIterations; count++) {
    for (int i = 0; i < (int)x.size(); i++) {
      for (int j = 0; j <= maximalNumberOfCopies;
           j++) res[j] =
          fabs(polynomial(x[i], j * a0 / ploidy, j * a1 / ploidy, j * a2 / ploidy,
                          j * a3 / ploidy) - y[i]);
      cluster[i] = get_min_index(res);
    }
    int npoints = 0;
    ...
    if (fabs(a0 - v(0)) + fabs(a1 - v(1)) + fabs(a2 - v(2)) + fabs(a3 - v(3)) ==
        0) {
      break;
    }
    a0 = v(0);
...
float runEM(const vector<float>& x,
            ...
            bool                 intercept,
            float                contamination) {
  if (contamination == 0) contamination = 0.3;  // starting from v11.2 - to
                                                // improve the fit
  float rmserror = -1;
  ...
  for (count = 0; count < NumberOfIterations; count++) {
    for (int i = 0; i < (int)x.size(); i++) {
      for (int j = 0; j <= maximalNumberOfCopies;
           j++) res[j] =
          fabs(polynomial(x[i], a,
                          (float(j) * (1 - contamination) + 2 * contamination) /
                          (ploidy * (1 - contamination) + 2 * contamination),
                          degree) - y[i]);
      cluster[i] = get_min_index(res);
    }
    int npoints = 0;
...
float runEMlog(const vector<float>& x,
               const vector<float>& y,
               double              *a,
               int                  degree,
               int                & realNumberOfIterations,
               int                  maximalNumberOfIterations,
               int                  ploidy,
               int                  maximalNumberOfCopies,
               bool                 intercept,
...
void processChrName(string& chr) {
  ...
  if ((pos = chr.find("chr", pos)) != string::npos) {
    ...
    if ((pos = chr.find("NC_", pos)) != string::npos) {
      ...
      else if (chr.compare("24") == 0) chr = "Y";
      else if (chr.compare("25") == 0) chr = "M";
      else if ((chr[0] == '0') || (chr[0] == '.')) chr = chr[1];
    }
  }
}
...
void filterWithQualities(string& pileupShort,
                         string& qualityS,
  ...
  for (unsigned int i = 0; i < pileupShort.length(); i++) {
    ...
    if (q >= minimalQualityPerPosition) newString += pileupShort[i];
  }
...
vector<int>merge_no_dups(const vector<int>& v1,
                         const vector<int>& v2);


void getBAFinfo(std::string  BAFValuesInTheSegment,
                float        copyNumber,
                float      & estimatedBAF,
                float      & fittedBAF,
                std::string& medianBAFSym,
                float      & uncertainty,
                float        normalContamination,
                int          ploidy,
                bool         noisyData,
                bool         ifHomoz,
  ...
  if (ifHomoz) {
    ...
    cout << "..too few points to fit the data, average copy number=" <<
      copyNumber << "\n";
      ...
      while (Bcount <= Acount) {
        ...
        if (normalContamination && (Bcount == 0)) {
          mu_ =
            max(mu_,
                normalContamination /
                (myCopyNumber * (1 - normalContamination) + 2 *
                 normalContamination));
        ...
        if (Bcount == Acount) {
          ...
          mu_ = (Bcount * (1. - normalContamination) + normalContamination) /
                (myCopyNumber * (1 - normalContamination) + 2 *
                 normalContamination);
          mu.push_back(mu_);
          mu.push_back(1 - mu_);
        }
        double LogLikelyHood = calculateLogLikelyHoodNormalMixtureForBAFs(BAFs,
                                                                          mu,
                                                                          middleComponentMinWeight,
                                                                          fixedMu,
  ...
  if (LogLikelyHoods.size() > 1) {
    ...
    uncertainty = 1. /
                  (LogLikelyHoods[LogLikelyHoods.size() - 1] -
                   LogLikelyHoods[LogLikelyHoods.size() - 2]);

    //     uncertainty =
    // exp(LogLikelyHoods[LogLikelyHoods.size()-2]-LogLikelyHoods[LogLikelyHoods.size()-1]);//since
    // v9.4 : p(secondBest)/p(best)
    if (copyNumbers.size() >= 1) {
      if ((copyNumbers[0] == 1) && (copyNumbers[1] == 2) && (copyNumber < 1.5) &&
          ((medianBAFSym.compare("AA") == 0) ||
           ((medianBAFSym.compare("AB") == 0) && (uncertainty > 0.1)))) {
        medianBAFSym = "A";
        estimatedBAF = 0;
        fittedBAF    = NA;
      }

      if ((copyNumbers[0] > 1) && (LogLikelyHoods.size() >= 3)) { // should be
                                                                  // always >=3,
                                                                  // but just in
                                                                  // case
        if (medianBAFSym[medianBAFSym.size() - 1] == 'A') {       // means AAAAA
          ...
          if (copyNumber - copyNumbers[0] >= 0.5) medianBAFSym += "A";

          // end recalculate uncertainty:
          // cout << LogLikelyHoods[LogLikelyHoods.size()-1]<< " " <<
          // LogLikelyHoods[LogLikelyHoods.size()-2] <<"\n";
          uncertainty = 1. /
                        (LogLikelyHoods[LogLikelyHoods.size() - 1] -
                         LogLikelyHoods[LogLikelyHoods.size() - 3]);
        }
      }

      if ((copyNumbers[0] > 1) && (winningCN != preferedCN)) { // check
                                                               // uncertainty
                                                               // for ambigous
                                                               // cases
        ...
        for (unsigned int jj = 0; jj < testedCN.size(); jj++) {
          if ((testedCN[jj] == preferedCN) &&
              (preferedCNLogLH < LogLikelyHoods_sec[jj])) {
            preferedCNLogLH = LogLikelyHoods_sec[jj];
            bestInd         = jj;
          }
        }
        float uncertWinVsPref = 1. /
                                (LogLikelyHoods[LogLikelyHoods.size() - 1] -
                                 preferedCNLogLH); // since v9.4
  ...
  if (uncertainty > 1) uncertainty = 100;
  else if (uncertainty > 0) uncertainty *= 100;
...
void getCopyNumbers(float copyNumber, vector<int>& copyNumbers) {
  ...
  if (copyNumber - lowCopy < minDiff) copyNumbers.push_back(lowCopy);
  else if (highCopy - copyNumber < minDiff) copyNumbers.push_back(highCopy);
  else {
...
void getCopyNumbers(float        copyNumber,
                    vector<int>& copyNumbers,
                    int          ploidy,
                    bool         noisyData) {
  int lowCopy  = int(floor(copyNumber));
  ...
  if (!noisyData) {
    ...
    if (lowCopy == ploidy) {
      if (copyNumber - lowCopy < minDiffNoNorm) copyNumbers.push_back(lowCopy);
      else if (highCopy - copyNumber < minDiff) copyNumbers.push_back(highCopy);
      else {
        copyNumbers.push_back(lowCopy);
        copyNumbers.push_back(highCopy);
      ...
      if (copyNumber - lowCopy < minDiff) copyNumbers.push_back(lowCopy);
      else if (highCopy - copyNumber < minDiffNoNorm) copyNumbers.push_back(
          highCopy);
      else {
        copyNumbers.push_back(lowCopy);
        copyNumbers.push_back(highCopy);
...
double calculateLogLikelyHoodNormalMixtureForBAFs(vector<float>X,
                                                  vector<float>mu,
                                                  float        middleComponentMinW,
                                                  bool         isMuFixed,
                                                  bool         CompleteGenomicsData)
{
  ...
  // START TMP
  //    std::ofstream file
  // ("/bioinfo/users/vboeva/Desktop/Neuroblastome/ANALYSES_CNG/Valentina_analysis/freec/rowBAFS_cn16.txt");
  //    for (int t = 0; t<N; t++) {
  //        file << X[t]<<"\n";
  //    }
  //    file.close();
  ...
  if (CompleteGenomicsData) { // correct mu:
    for (int i = 0; i < numberOfStates; i++) {
      if (mu[i] != HOMOZYG_MEAN) mu[i] *= 0.82;
    }
  }
  ...
  if (numberOfStates == 2) {
    ...
    cerr <<
      "Warning: in fitting by a mixture of normals, the maximum number of compontents is 4. Using "
         << numberOfStates << " components\n";
    return NA;
  }
  ...
  for (int t = 0; t < N; t++) {
    ...
    for (int s = 0; s < numberOfStates; s++) {
      h[s][t] = Omega.at(s) *
                NormalDistributionDensity(X[t], mu[s], sigma[s]) / sum_t;

      if ((t == 0) || (t == 1) || (t == 376) || (t == 377)) {
        //                   cout << s << "," << t << ", X[t]=" << X[t] << ",
        // h[s][t] =" <<h[s][t] << ", Omega.at(s)=" <<Omega.at(s)<< "\n";
      }
    }
  }
  ...
  //    for (int s = 0; s<numberOfStates; s++) {
  //        for (int t = 0; t<N; t++) {
  //            LogLikelyHood_iPlusOne +=
  // (log(Omega[s])+log(NormalDistributionDensity(X[t],mu[s],sigma[s])))*h[s][t];
  //        }
  //    }
  ...
  while (fabs(LogLikelyHood_i - LogLikelyHood_iPlusOne) >= 0.0001 &&
    ...
    if (isMuFixed && (numberOfStates == 3)) {
      ...
      if (sigma[2] > maxSigma) sigma[2] = maxSigma;
    }
    ...
    if (isMuFixed && (numberOfStates == 4)) {
      ...
      if ((Omega[2] < minMinOmega) || (Omega[3] < minMinOmega)) {
        if ((Omega[2] < minMinOmega) && (Omega[3] < minMinOmega)) { // it is not
                                                                    // possible
      ...
      if (sigma[3] > maxSigma) sigma[3] = maxSigma;
    }
...
double NormalDistributionDensity(double x, double mu, double sigma) {
  ...
  if (sigma) result = 0.398942449 / sigma * exp(
...
std::string getNormalBAFforPloidy(int ploidy) {
  if (ploidy > 1) {
    ...
    for (int i = 1; i < Acount; i++) BAF += "A";

    for (int i = 0; i < Bcount; i++) BAF += "B";
    return BAF;
  } else {
    if (ploidy == 1) return "A";
  }
  return "";
}
...
bool getELANDinfo(std::string  line,
                  std::string& chr1,
                  std::string& chr2,
                  std::string& orient1,
                  std::string& orient2,
...
bool getSAMinfo(const char  *line,
                std::string& chr1,
                std::string& chr2,
                char       & orient1,
                char       & orient2,
...
bool getSAMinfo(const std::string& line,
                std::string      & chr1,
                std::string      & chr2,
                std::string      & orient1,
                std::string      & orient2,
  ...
  if ((chr1.compare("*") == 0) || (chr2.compare("*") == 0)) return false;

  if (chr2.compare("=") == 0) chr2 = chr1;


  if (atoi(strs[1].c_str()) & 0x0010) orient1 = "R";
  else orient1 = "F";

  if (atoi(strs[1].c_str()) & 0x0020) orient2 = "R";
  else orient2 = "F";
  ...
  if (chr1.compare(chr2) != 0) return false;  // Can be avoided, TODO
...
int calculateTotalLength(std::vector<int>lefts, std::vector<int>rights) {
  if ((lefts.size() == 0) || (rights.size() == 0) ||
      (lefts.size() != rights.size())) return 0;
  ...
  for (int unsigned i = 1; i < rights.size(); i++) {
    ...
    if (lefts[i] > rights[i - 1]) length += rights[i] - lefts[i] + 1;
...
double expm1(double x) {
  ...
  else return exp(x) - 1.0;
}

double log1p(double x) {
  if (fabs(x) < 1e-5) return x - x * x / 2 + x * x * x / 3;
  else return log(x + 1);
}
...
MateOrientation getMateOrientation(std::string const& matesOrientation)
{
  ...
  if ((matesOrientation.compare("FF") == 0) ||
      (matesOrientation.compare("RR") == 0) ||
      (matesOrientation.compare("FF/RR") == 0) ||
      (matesOrientation.compare("RR/FF") == 0)) {
    return SOLID_MATE_PAIRS;
  }
  cerr << "Error: you have set an unknown read orientation: \"" <<
    matesOrientation <<
    "\" ; it does not match 0, RF, FR or FF; please correct your config file.\n";
  exit(1);
...
InputFormat getInputFormat(std::string const& inputFormat)
{
  ...
  if ((inputFormat.compare("arachne") == 0) ||
      (inputFormat.compare("BED") == 0) || (inputFormat.compare("bed") == 0) ||
      (inputFormat.compare("ARACHNE") == 0)) {
    return ARACHNE_BED_INPUT_FORMAT;
  }

  if ((inputFormat.compare("SOAP") == 0) || (inputFormat.compare("soap") == 0) ||
      (inputFormat.compare("Soap") == 0)) {
